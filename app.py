...
from core.middleware import SecurityHeadersMiddleware, require_admin
...
if AUTH_ENABLED:
    ...
    AUTH_EXEMPT_PATTERNS = [
        ...
        # Artifact previews (routes/artifact_routes.py): loaded inside a
        # sandboxed iframe whose opaque origin may not send the session
        # cookie. The unguessable 128-bit id IS the credential (same model
        # as the webhook tokens above); unknown ids 404 and artifacts are
        # in-memory only.
        _re.compile(r"^/artifact/[A-Za-z0-9_-]{16,}$"),
    ]
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. PAIOS's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-PAIOS-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-PAIOS-Owner") or "").strip()
            ...
            if LOCALHOST_BYPASS and _is_trusted_loopback(request):
                _auth_mgr = getattr(request.app.state, "auth_manager", None) or auth_manager
                _user = "admin"
                if _auth_mgr and getattr(_auth_mgr, "users", {}):
                    _user = next(iter(_auth_mgr.users.keys()))
...
# Emoji SVG proxy (same-origin, lazy-cached OpenMoji "black" line art) — lets
# the chat render emojis as flat monochrome SVG instead of system color glyphs.
# OpenMoji is CC BY-SA 4.0 and REQUIRES attribution wherever it is served; the
# credit line lives in ACKNOWLEDGMENTS.md, NOTICE and README.md. Full licence in
# licenses/OpenMoji-CC-BY-SA-4.0.txt. (This comment used to say "Twemoji", which
# is a different project under a different licence — do not reintroduce that.)
from routes.emoji_routes import setup_emoji_routes
app.include_router(setup_emoji_routes())
from routes.usage_routes import setup_usage_routes
app.include_router(setup_usage_routes())
from routes.cli_llm_routes import setup_cli_llm_routes
app.include_router(setup_cli_llm_routes())
...
# Global file/folder name index (Agents tab: Sync + Search)
from routes.file_index_routes import setup_file_index_routes
app.include_router(setup_file_index_routes())
...
# Video generation (pluggable open-source text-to-video endpoint, model_type="video")
from routes.video_routes import setup_video_routes
app.include_router(setup_video_routes())

# Local AI hub — one-click download/activate of local models across all modalities
from routes.models_hub_routes import setup_models_hub_routes
app.include_router(setup_models_hub_routes())
...
# Crew — local multi-agent mesh (Supervisor + Planner/Coder/Retriever/Ops).
# Local-first routing, deny-by-default per-agent tools, plan-approval gate,
# run audit. Reuses the existing agent loop / Cowork / RAG (services/crew).
from routes.crew_routes import setup_crew_routes
app.include_router(setup_crew_routes())

# Code — project-aware coding agent (repo map + AGENTS.md context injected into
# the Cowork engine). services/code/repomap + routes/code_routes.
from routes.code_routes import setup_code_routes
app.include_router(setup_code_routes())

# Activity / Capacity — live "what's running + how much headroom" (services/activity).
from routes.activity_routes import setup_activity_routes
app.include_router(setup_activity_routes())

# Storage — disks, model storage, Pi footprint (services/storage).
from routes.storage_routes import setup_storage_routes
app.include_router(setup_storage_routes())

# Agents hub — chat-based agent management over the TaskScheduler engine.
from routes.agents_hub_routes import setup_agents_hub_routes
app.include_router(setup_agents_hub_routes())

# Gateways — Telegram remote access (off by default; services/gateway).
from routes.gateway_routes import setup_gateway_routes
app.include_router(setup_gateway_routes())

# Artifacts — sandboxed live preview of generated HTML/SVG/React (Claude-style).
from routes.artifact_routes import setup_artifact_routes
app.include_router(setup_artifact_routes())

# Mail Guard — local AI email triage across all accounts (services/mailguard).
from routes.mailguard_routes import setup_mailguard_routes
app.include_router(setup_mailguard_routes())
...
async def serve_help(request: Request):
    """The interactive MyPaiOS help guide (MyPaiOS-Help-Guide.html at the repo root)."""
    return _serve_html_with_nonce(request, abs_join(BASE_DIR, "MyPaiOS-Help-Guide.html"))


_help_pdf_lock = asyncio.Lock()


@app.get("/help.pdf")
async def serve_help_pdf(request: Request):
    """The help guide exported as a PDF (rendered locally via Playwright
    Chromium). Cached next to the data dir and re-rendered only when the
    guide file changes — first call takes ~5-10s, later ones are instant."""
    import os as _os
    src = abs_join(BASE_DIR, "MyPaiOS-Help-Guide.html")
    from src.constants import DATA_DIR as _DD
    cache_dir = _os.path.join(_DD, ".cache")
    _os.makedirs(cache_dir, exist_ok=True)
    out = _os.path.join(cache_dir, "MyPaiOS-Help-Guide.pdf")
    async with _help_pdf_lock:
        if (not _os.path.exists(out)
                or _os.path.getmtime(out) < _os.path.getmtime(src)):
            ...
            async with async_playwright() as p:
                ...
                try:
                    ...
                    # Print prep: light theme, all accordions open, no sidebar/
                    # topbar chrome, content full-width across A4 pages.
                    ...
                    await page.evaluate("""() => {
                        ...
                        css.textContent = `
                          .sidebar, .topbar, .menutoggle { display: none !important; }
                          /* Flex containers don't fragment across PDF pages in
                             Chromium — flatten to block or the PDF truncates. */
                          .wrap, .content, .inner { display: block !important; }
                          .content { padding: 0 !important; }
                          .inner { max-width: 100% !important; }
                          * { scroll-behavior: auto !important; }
                          /* break-inside:avoid ONLY on elements guaranteed to
                             fit one page — on taller ones (tables, open
                             accordions) Chromium truncates the whole PDF. */
                          .card, pre, tr { break-inside: avoid; }
                        `;
    ...
    return FileResponse(out, media_type="application/pdf",
                        filename="MyPaiOS-Help-Guide.pdf")
...
@app.get("/api/pi-log")
async def pi_log_tail(request: Request, n: int = 200, kind: str = ""):
    """Last N events from the light 'what Pi does' log (admin-only).
    kind filters: tool | nudge | http | turn. Also greppable at logs/pi-events.jsonl."""
    require_admin(request)
    from services import pi_log
    return {"events": pi_log.tail(min(max(int(n), 1), 1000), (kind or "").strip())}
...
async def _startup_event():
    ...
    # `PAIOS_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("PAIOS_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (PAIOS_INPROCESS_TASKS=0); "
    ...
    # File index (Agents tab) — populate ~20s after boot, then re-sync every 6h.
    # Manual re-sync is available via POST /api/file-index/sync.
    async def _file_index_sync_loop():
        try:
            from src.file_index import get_file_index_manager
            mgr = get_file_index_manager()
            await asyncio.sleep(8)
            logger.info("file_index: initial sync starting (%d roots)", len(mgr.get_roots()))
            res = await asyncio.to_thread(mgr.sync_all)
            logger.info("file_index: initial sync done: %s", res)
        except Exception as e:
            logger.warning("file_index initial sync failed: %s", e, exc_info=True)
            return
        while True:
            try:
                await asyncio.sleep(6 * 3600)
                await asyncio.to_thread(mgr.sync_all)
            except Exception as e:
                logger.warning("file_index sync loop error: %s", e, exc_info=True)
                await asyncio.sleep(300)
    logger.info("file_index: scheduling sync loop")
    _startup_tasks.append(asyncio.create_task(_file_index_sync_loop()))

    # Telegram gateway — idles until enabled in settings (services/gateway).
    try:
        from services.gateway.telegram_gateway import run_loop as _tg_loop
        _startup_tasks.append(asyncio.create_task(_tg_loop()))
    except Exception as _e:
        logger.warning(f"telegram gateway not started: {_e}")

    # Mail Guard — email triage runner; idles until enabled in settings.
    try:
        from services.mailguard.runner import run_loop as _mg_loop
        _startup_tasks.append(asyncio.create_task(_mg_loop()))
    except Exception as _e:
        logger.warning(f"mail guard not started: {_e}")
