...
TOKEN_TTL = int(os.getenv("PAIOS_SESSION_TTL_DAYS", "7")) * 86400          # default 7 days
# "Remember me" issues a long-lived token (default 90 days). Tokens are random
# (secrets.token_hex) and stored server-side in data/sessions.json — no password
# is ever stored — so they persist across restarts.
REMEMBER_ME_TTL = int(os.getenv("PAIOS_REMEMBER_ME_DAYS", "90")) * 86400    # default 90 days
...
class AuthManager:
    ...
    def totp_get_provisioning_uri(self, username: str, secret: str) -> str:
        ...
        return totp.provisioning_uri(name=username, issuer_name="PAIOS")
    ...
    def create_session(self, username: str, password: str, remember_me: bool = False) -> Optional[str]:
        """Verify credentials and return a session token, or None.

        When remember_me is set, the token lives REMEMBER_ME_TTL (default 90
        days) instead of TOKEN_TTL (7 days). The token is random and stored
        server-side (data/sessions.json), so it survives restarts.
        """
        username = username.strip().lower()
        if not self.verify_password(username, password):
            return None
        token = secrets.token_hex(32)
        ttl = REMEMBER_ME_TTL if remember_me else TOKEN_TTL
        with self._sessions_lock:
