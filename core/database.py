...
class Session(TimestampMixin, Base):
    ...
    total_cost_usd = Column(Float, default=0.0)   # cloud spend for this session (USD)
...
class ModelEndpoint(TimestampMixin, Base):
    ...
    model_meta = Column(Text, nullable=True)       # JSON {model_id: {pricing (USD/1M), context_length, modalities, supported_parameters, fetched_at}}
...
class UsageLedger(Base):
    """One row per LLM generation — the spend ledger behind cost badges,
    session totals, and the Usage dashboard. Contains counts and costs only,
    ...
    model = Column(String, nullable=True, index=True)   # ACTUAL serving model (post-fallback)
    input_tokens = Column(Integer, default=0)
    output_tokens = Column(Integer, default=0)
    cached_tokens = Column(Integer, default=0)
    cache_write_tokens = Column(Integer, default=0)
    reasoning_tokens = Column(Integer, default=0)
    cost_usd = Column(Float, nullable=True)             # None = unknown price
    cost_source = Column(String, nullable=True)         # reported|catalog|litellm|override|estimated|partial|free|local|reconciled
    response_id = Column(String, nullable=True)         # provider generation id (reconciliation)


def _migrate_add_pricing_columns():
    """Add model_meta (model_endpoints) + total_cost_usd (sessions)."""
    import sqlite3
    db_path = DATABASE_URL.replace("sqlite:///", "")
    if not os.path.exists(db_path):
        return
    try:
        conn = sqlite3.connect(db_path)
        cols = [r[1] for r in conn.execute("PRAGMA table_info(model_endpoints)").fetchall()]
        if cols and "model_meta" not in cols:
            conn.execute("ALTER TABLE model_endpoints ADD COLUMN model_meta TEXT")
            logging.getLogger(__name__).info("Migrated: added 'model_meta' to model_endpoints")
        cols = [r[1] for r in conn.execute("PRAGMA table_info(sessions)").fetchall()]
        if cols and "total_cost_usd" not in cols:
            conn.execute("ALTER TABLE sessions ADD COLUMN total_cost_usd REAL DEFAULT 0")
            logging.getLogger(__name__).info("Migrated: added 'total_cost_usd' to sessions")
        conn.commit()
        conn.close()
    except Exception as e:
        logging.getLogger(__name__).warning(f"pricing columns migration failed: {e}")
