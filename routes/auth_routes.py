...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    async def first_run_setup(body: SetupRequest, request: Request):
        ...
        SECURITY (first-comer-wins takeover, audited 2026-07-20): this
        endpoint hard-refuses once ANY account exists, at two layers —
        (1) the `is_configured` check below rejects with 400, and
        (2) `AuthManager.setup()` re-checks `is_configured` under
        `_setup_lock`, so two concurrent requests racing an empty
        auth.json cannot both create an admin (core/auth.py). Because
        the very first account created is always an admin, "any account
        exists" implies "an admin exists" — a remote caller can never
        use /setup to add an admin to an already-configured install.
        """
    ...
    async def login(body: LoginRequest, request: Request, response: Response):
        ...
        token = await asyncio.to_thread(auth_manager.create_session, username, body.password, body.remember)
        if not token:
            raise HTTPException(401, "Invalid credentials")
        cookie_kwargs = dict(
            key=SESSION_COOKIE,
        ...
        if body.remember:
            from core.auth import REMEMBER_ME_TTL
            cookie_kwargs["max_age"] = REMEMBER_ME_TTL  # 90 days (see core/auth.py)
    ...
    # Keys the pre-auth UI genuinely needs from GET /settings. The endpoint is
    # auth-exempt (the login page prefetches it and app pages whose session
    # expired still call it), so UNAUTHENTICATED callers get ONLY these keys —
    # an explicit allowlist, not a denylist. A denylist scrub leaks any secret
    # whose name doesn't look secret-shaped; with an allowlist a new settings
    # key is private-by-default until deliberately added here. Consumers:
    #   keybinds                   static/js/keyboard-shortcuts.js
    #   tts_enabled/tts_provider   static/app.js + static/js/tts-ai.js
    #   tts_voice/tts_speed        TTS playback prefs
    #   search_provider            static/js/search.js (provider label)
    _PREAUTH_SETTINGS_ALLOWLIST = (
        "keybinds",
        "tts_enabled",
        "tts_provider",
        "tts_voice",
    ...
    async def get_settings(request: Request):
        ...
        non-admins get a scrubbed copy with secret keys blanked (they need
        keybinds + TTS prefs, so it stays callable without admin); and
        unauthenticated callers get only _PREAUTH_SETTINGS_ALLOWLIST."""
        user = _get_current_user(request)
        ...
        # Unauthenticated: allowlist projection first, then the deep scrub as
        # defense-in-depth (e.g. a secret nested under an allowlisted key).
        allowed = {k: settings[k] for k in _PREAUTH_SETTINGS_ALLOWLIST if k in settings}
        return scrub_settings(allowed)
    ...
    async def test_integration_route(integration_id: str, request: Request):
        ...
        if preset == "ntfy":
            ...
            # base URL (e.g. `http://host:8091/paios`) — otherwise
            # the topic gets appended after the path and we publish to
            # `/paios/paios` (which ntfy 404s on). ntfy itself
            ...
            try:
                async with httpx.AsyncClient(timeout=8.0) as client:
                    r = await client.post(
                        full_url,
                        content="Connectivity test from PAIOS. If you see this on your phone, ntfy is wired up correctly.",
                        headers=headers,
                    )
                if r.is_success:
