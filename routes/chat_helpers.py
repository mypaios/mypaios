...
def enrich_metrics_with_cost(metrics: dict, endpoint_url: str = None) -> dict:
    """Fill cost_usd/cost_source (+provider) on a metrics dict.

    Provider-reported cost (OpenRouter usage.cost) wins and is never
    overwritten; otherwise the layered PriceBook computes it, and an unknown
    price yields cost_source='unknown' — the UI shows 'unknown', never a guess.
    Local endpoints are free ('local'). Estimated token counts produce
    cost_source='estimated'.
    """
    try:
        from services.pricing import get_price, compute_cost
        from services.pricing.pricebook import is_local_url
        from src.llm_core import _detect_provider
        provider = _detect_provider(endpoint_url or "")
        metrics.setdefault("provider", provider)
        if metrics.get("cost_usd") is not None:          # reported — the truth
            metrics.setdefault("cost_source", "reported")
            return metrics
        model = metrics.get("served_model") or metrics.get("model") or ""
        local = provider == "ollama" or is_local_url(endpoint_url)
        model_meta = None
        if not local and endpoint_url:
            try:
                from core.database import SessionLocal as _SL, ModelEndpoint as _ME
                _db = _SL()
                try:
                    base = endpoint_url.split("/chat/completions")[0].rstrip("/")
                    for ep in _db.query(_ME).filter(_ME.model_meta.isnot(None)).all():
                        if base.startswith((ep.base_url or "").rstrip("/")) or (ep.base_url or "").rstrip("/").startswith(base):
                            import json as _json
                            model_meta = (_json.loads(ep.model_meta) or {}).get(model)
                            if model_meta:
                                break
                finally:
                    _db.close()
                ...
                model_meta = None
        price = get_price(model, model_meta=model_meta, is_local=local)
        cost = compute_cost(metrics, price, provider)
        if cost is None:
            metrics["cost_source"] = "unknown"
        else:
            metrics["cost_usd"] = cost
            src = (price or {}).get("source", "unknown")
            metrics["cost_source"] = ("estimated" if metrics.get("usage_source") == "estimated" and src not in ("local",) else src)
    except Exception as e:
        logger.debug(f"cost enrichment failed: {e}")
    return metrics


def accumulate_token_usage(session_id: str, metrics: dict, message_id: str = None):
    """Add token counts + cost to the session totals AND write the spend-ledger
    row (counts + cost only, never content)."""
    ...
    try:
        ...
        if db_s:
            ...
            if cost:
                db_s.total_cost_usd = (db_s.total_cost_usd or 0.0) + float(cost)
        try:
            from core.database import UsageLedger
            db.add(UsageLedger(
                owner=getattr(db_s, "owner", None) if db_s else None,
                session_id=session_id,
                message_id=message_id,
                provider=metrics.get("provider"),
                model=metrics.get("served_model") or metrics.get("model"),
                input_tokens=in_t, output_tokens=out_t,
                cached_tokens=metrics.get("cached_tokens", 0) or 0,
                cache_write_tokens=metrics.get("cache_write_tokens", 0) or 0,
                reasoning_tokens=metrics.get("reasoning_tokens", 0) or 0,
                cost_usd=(float(cost) if cost is not None else None),
                cost_source=metrics.get("cost_source"),
                response_id=metrics.get("response_id"),
            ))
        except Exception as le:
            logger.debug(f"usage ledger write failed: {le}")
        db.commit()
    except Exception:
        db.rollback()
    finally:
...
def run_post_response_tasks(
    ...
    *,
    ...
    # Obsidian brain — write this turn back into the user's own vault (transcript
    # + extracted outputs + summary) so knowledge accumulates and is retrievable
    # later. Per-user + isolated; safe no-op if the owner has no vault configured.
    if not incognito and not compare_mode and bool(uprefs.get("obsidian_brain", True)):
        try:
            from src.obsidian_brain import record_turn
            record_turn(
                owner=owner, sess=sess, user_message=message,
                assistant_reply=full_response, uprefs=uprefs,
            )
        except Exception as e:
            logger.warning(f"[obsidian-brain] write-back skipped: {e}")
    ...
    # Token accumulation (+ cost + spend ledger). Enrichment is idempotent —
    # a provider-reported cost (OpenRouter usage.cost) is never overwritten.
    if last_metrics:
        enrich_metrics_with_cost(last_metrics, getattr(sess, "endpoint_url", None))
