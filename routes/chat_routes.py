...
def setup_chat_routes(
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        _image_intent = False  # chat-mode "generate an image of …" → fast image path
        _tool_intent = _classify_tool_intent(message) if isinstance(message, str) else None
        if chat_mode == "chat" and _tool_intent and _tool_intent.needs_tools:
            if _tool_intent.category == "image":
                # Don't run the whole agent loop for an image request — route it
                # straight to the do_generate_image fast path below.
                _image_intent = True
                logger.info("chat image-intent → direct image generation (%s)", _tool_intent.reason)
            else:
                chat_mode = "agent"
                auto_escalated = True
                logger.info(
        ...
        ctx = await build_chat_context(
            ...
            # When a workspace is active, inject that project's PI.md/AGENTS.md
            # (global PI.md is injected regardless).
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            if _img_session or _image_intent:
                from src.settings import get_setting
                ...
                yield ": heartbeat\n\n"
                # Image SESSION → use the session's (image) model. Chat-mode image
                # INTENT → no model line so do_generate_image auto-detects the image
                # endpoint (sess.model here is the text model, e.g. gpt-oss).
                _img_content = f"{_user_msg}\n{sess.model}" if _img_session else _user_msg
                _img_result = await do_generate_image(_img_content, session, owner=_user)
                        ...
                        if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                            try:
                                ...
                                if "delta" in data:
                                    ...
                                    # Cost: provider-reported wins; else PriceBook.
                                    from routes.chat_helpers import enrich_metrics_with_cost
                                    enrich_metrics_with_cost(last_metrics, sess.endpoint_url)
        ...
        async def _safe_stream() -> AsyncGenerator[str, None]:
            ...
            _act = activity.register("chat", None, (message or "")[:60],
                                     ref=session, cancel=lambda: agent_runs.stop(session))
            try:
                async for chunk in stream_with_save():
                    yield chunk
            finally:
                _active_streams.pop(session, None)
                activity.unregister(_act)
