                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in PAIOS (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in PAIOS" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "paios_kind" if "paios_kind" in cols else "'scheduled' AS paios_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                paios_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-PAIOS-Origin"] = "paios-ui"
                outer["X-PAIOS-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", paios_kind or "scheduled")[:64]
                outer["X-PAIOS-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `paios-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `PAIOS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `paios-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("PAIOS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `PAIOS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (PAIOS_INPROCESS_POLLERS=0); "
            "drive `paios-mail poll-scheduled` externally."
        )
        return
    import asyncio
