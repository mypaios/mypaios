...
def _apply_paios_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-PAIOS-Origin"] = PAIOS_MAIL_ORIGIN
    if kind:
        msg["X-PAIOS-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-PAIOS-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        ok = True
        try:
            conn, _ = _pooled_connect(account_id, owner=owner)
            ...
            if filter_ == "unread":
                ...
                # Prefer the PAIOS marker header, but include the subject
                # fallback too. The fallback uses a distinct PAIOS prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-PAIOS-Kind "reminder" SUBJECT "Reminder (PAIOS):"{from_clause})',
                )
            ...
            if conn:
                try:
                    _pooled_release(account_id, conn, ok=ok, owner=owner)
                except Exception:
                    pass
    ...
    @router.delete("/paios/reminders")
    async def delete_paios_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as PAIOS reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-PAIOS-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (PAIOS):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (PAIOS):")})'))
                            # Legacy reminders created before the PAIOS
            ...
            logger.error(f"delete_paios_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", paios_kind=None, paios_ref=None,
    ):
        ...
        _apply_paios_headers(outer, paios_kind or "scheduled", paios_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, paios_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("paios_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="paios.local")
        ...
        if req.paios_kind:
            _apply_paios_headers(outer, req.paios_kind)
