...
def _container_loopback_reachable(base_url: str, timeout: float = 0.2) -> bool:
    ...
    This distinguishes "a model server running alongside PAIOS in the same
...
def _rewrite_loopback_for_docker(base_url: str, *, container_local: bool = False) -> str:
    ...
    default) otherwise targets the PAIOS container itself, so the probe gets
    a connection error and the endpoint is rejected with a misleading "No
    models found for that provider/key".

    Cookbook local serves are the opposite case: PAIOS started the model
...
# Side-channel: the last rich per-model metadata captured by _probe_endpoint,
# keyed by normalized base URL. OpenRouter (and Together/xAI later) return
# pricing/context/capabilities in the SAME /models response we already fetch —
# persist sites read this via _meta_json_for() right after updating
# cached_models, so no probe call-site signatures change.
_probe_meta_cache: dict = {}


def _extract_model_meta(entries) -> dict:
    """Pull {model_id: {pricing(USD/1M), context_length, modalities,
    supported_parameters, fetched_at}} out of an OpenAI-format /models list.
    Returns {} when the catalog carries no rich fields (plain providers)."""
    import time as _t
    meta = {}
    try:
        from services.pricing import normalize_openrouter_pricing
        for m in entries or []:
            if not isinstance(m, dict) or not m.get("id"):
                continue
            row = {}
            if isinstance(m.get("pricing"), dict):
                pr = normalize_openrouter_pricing(m["pricing"])
                if pr:
                    row["pricing"] = pr
                elif all(str(v) in ("0", "0.0", "") or v is None for v in m["pricing"].values()):
                    row["pricing"] = {"in_per_m": 0.0, "out_per_m": 0.0}  # explicit free
            if m.get("context_length"):
                row["context_length"] = m["context_length"]
            arch = m.get("architecture") or {}
            if isinstance(arch, dict) and arch.get("input_modalities"):
                row["modalities"] = arch.get("input_modalities")
            if m.get("supported_parameters"):
                row["supported_parameters"] = m["supported_parameters"]
            if row:
                row["fetched_at"] = int(_t.time())
                meta[m["id"]] = row
    except Exception:
        return {}
    return meta
...
def _meta_json_for(base_url: str):
    """JSON string of the last captured catalog metadata for this base, or None."""
    try:
        from src.endpoint_resolver import resolve_url
        base = resolve_url(_normalize_base(base_url))
        meta = _probe_meta_cache.get(base)
...
def _probe_endpoint(base_url: str, api_key: str = None, timeout: int = 5) -> List[str]:
    ...
    try:
        ...
        models = [m.get("id") for m in (data.get("data") or []) if m.get("id")]
        _meta = _extract_model_meta(data.get("data"))
        if _meta:
            _probe_meta_cache[base] = _meta
        ...
        if 300 <= r.status_code < 400:
            ...
            if loc.startswith("/login") or "/login" in loc:
                return {
                    ...
                    "error": "That is PAIOS, not a model server. Use the Ollama URL, usually http://host.docker.internal:11434/v1 in Docker.",
                }
...
def setup_model_routes(model_discovery):
                                ...
                                if ids:
                                    for ep_id in endpoint_ids:
                                        ...
                                        if ep_obj:
                                            ...
                                            _mm = _meta_json_for(groups[key]["base"])
                                            if _mm:
                                                ep_obj.model_meta = _mm
                                            changed = True
    ...
    def probe_selected(request: Request, request_body: dict = Body(...)):
        ...
        try:
            ...
            for item in models_to_probe:
                ...
                timeout_val = 120 if _classify_endpoint(base) == "local" else 15
                result = _probe_single_model(base, ep_data.get("api_key"), model_id, timeout=timeout_val, with_tools=_with_tools)
        ...
        def _stream():
            ...
            for ep in ep_data:
                ...
                for model_id in models:
                    total += 1
                    timeout_val = 120 if _classify_endpoint(base) == "local" else 15
                    result = _probe_single_model(base, ep.get("api_key"), model_id, timeout=timeout_val)
    ...
    def create_model_endpoint(
        ...
        # server. Cookbook local serves are launched inside PAIOS itself, so
            ...
            if existing:
                ...
                if should_probe:
                    ...
                    if probed_models:
                        ...
                        _mm = _meta_json_for(base_url)
                        if _mm:
                            existing.model_meta = _mm
                        changed = True
                if changed:
                    _db_dedup.commit()
                    _invalidate_models_cache()
        ...
        try:
            ...
            ep = ModelEndpoint(
                ...
                model_meta=_meta_json_for(base_url),
    ...
    def probe_endpoint_models(ep_id: str, request: Request):
        ...
        def _stream():
            ...
            for mid in chat_models:
                timeout_val = 120 if _classify_endpoint(base) == "local" else 15
                result = _probe_single_model(base, ep_data["api_key"], mid, timeout=timeout_val)
                result["model"] = mid
                result["type"] = "probe_result"
