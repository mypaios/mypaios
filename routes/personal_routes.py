...
MAX_PERSONAL_UPLOAD_BYTES = int(
    os.getenv("PAIOS_PERSONAL_UPLOAD_MAX_BYTES", str(25 * 1024 * 1024))
)
...
def setup_personal_routes(personal_docs_manager, rag_manager, rag_available):
    ...
    def _resolve_allowed_personal_dir(directory: str) -> str:
        """Resolve a user-supplied personal-docs path under an allowed root.

        Allowed roots are PERSONAL_DIR plus any absolute paths listed in the
        PAIOS_DOC_ROOTS env var (comma-separated) — e.g. an Obsidian vault that
        lives outside data/personal_docs. Defaults to the original behavior
        (PERSONAL_DIR only) when PAIOS_DOC_ROOTS is unset.
        """
        if not directory:
            raise HTTPException(400, "Directory path is required")

        # realpath (not abspath) so a symlink that points outside an allowed
        # root is resolved before the commonpath confinement check below;
        ...
        allowed_roots = [base_abs]
        for extra in os.getenv("PAIOS_DOC_ROOTS", "").split(","):
            extra = extra.strip()
            if extra:
                allowed_roots.append(os.path.realpath(os.path.expanduser(extra)))
        ...
        for root in allowed_roots:
            try:
                ...
                continue  # different drive (Windows) etc.
        raise HTTPException(
            403,
            "Directory must be inside personal documents or an allowed PAIOS_DOC_ROOTS path",
        )
