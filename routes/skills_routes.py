...
def setup_skills_routes(skills_manager: SkillsManager) -> APIRouter:
    ...
    # ── Bundled files + import/export (the portability + authoring half) ──

    def _require_skill(user, skill_id):
        sk = next((s for s in skills_manager.load(owner=user) if s.get("name") == skill_id), None)
        if not sk:
            raise HTTPException(404, "Skill not found")
        return sk

    @router.get("/{skill_id}/files")
    async def list_skill_files_route(skill_id: str, request: Request):
        user = _owner(request)
        _require_skill(user, skill_id)
        return {"files": skills_manager.list_skill_files(skill_id, owner=user)}

    @router.post("/{skill_id}/files")
    async def write_skill_file_route(skill_id: str, request: Request):
        user = _owner(request)
        _require_skill(user, skill_id)
        body = await request.json()
        rel = (body or {}).get("path", "").strip()
        if not rel:
            raise HTTPException(400, "path is required (e.g. 'scripts/build.py')")
        content = (body or {}).get("content", "")
        if (body or {}).get("base64"):
            import base64
            try:
                data = base64.b64decode(content)
            except Exception as e:
                raise HTTPException(400, f"bad base64: {e}")
            written = skills_manager.write_skill_file(skill_id, rel, data, owner=user, binary=True)
        else:
            written = skills_manager.write_skill_file(skill_id, rel, content, owner=user)
        if written is None:
            raise HTTPException(400, "Unsafe path, or SKILL.md cannot be written here")
        return {"ok": True, "path": written}

    @router.delete("/{skill_id}/files")
    async def delete_skill_file_route(skill_id: str, request: Request, path: str):
        user = _owner(request)
        _require_skill(user, skill_id)
        if not skills_manager.delete_skill_file(skill_id, path, owner=user):
            raise HTTPException(400, "Could not delete (not found or unsafe path)")
        return {"ok": True}

    @router.post("/{skill_id}/run-script")
    async def run_skill_script_route(skill_id: str, request: Request):
        user = _owner(request)
        sk = _require_skill(user, skill_id)
        if sk.get("source") == "imported" and not sk.get("trusted_exec"):
            raise HTTPException(403, "Imported skill is quarantined; set trusted_exec to allow run_script.")
        body = await request.json()
        script = (body or {}).get("script", "").strip()
        if not script:
            raise HTTPException(400, "script is required")
        args = (body or {}).get("args") or []
        from services.memory.skill_runner import run_skill_script
        import os as _os
        res = run_skill_script(_os.path.dirname(sk["path"]), script,
                               args if isinstance(args, list) else [])
        return res

    @router.get("/{skill_id}/export")
    async def export_skill_route(skill_id: str, request: Request):
        user = _owner(request)
        _require_skill(user, skill_id)
        data = skills_manager.export_skill_zip(skill_id, owner=user)
        if data is None:
            raise HTTPException(404, "Skill not found")
        import io
    ...
    async def import_skill_route(request: Request, file: UploadFile = File(...)):
        ...
        if len(data) > 25 * 1024 * 1024:
            raise HTTPException(400, "Skill zip too large (max 25 MB)")
        res = skills_manager.import_skill_zip(data, owner=user)
        if not res.get("ok"):
            raise HTTPException(400, res.get("error", "import failed"))
        _fire_skill_added(user)
        return res
