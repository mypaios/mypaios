...
def _maybe_cascade_calendar_event(task) -> None:
    ...
    if task.owner:
        headers["X-PAIOS-Owner"] = task.owner
...
def setup_task_routes(task_scheduler) -> APIRouter:
    ...
    async def create_task(request: Request, req: TaskCreate):
        ...
        db = SessionLocal()
        try:
            # Idempotent create: if an EXACT duplicate already exists for this
            # owner (same name + prompt + action), return it instead of inserting
            # a second identical row. This is what produced the duplicate
            # "AI News Summary" agents — re-running the create flow minted a fresh
            # UUID each time. Only exact dupes are blocked, so intentional
            # same-name-different-config agents are unaffected.
            dup = db.query(ScheduledTask).filter(
                ScheduledTask.owner == user,
                ScheduledTask.name == name,
                ScheduledTask.prompt == req.prompt,
                ScheduledTask.action == req.action,
            ).first()
            if dup is not None:
                logger.info("create_task: returning existing exact-duplicate %r for owner %s", name, user)
                return _task_to_dict(dup)
