...
def _as_bool(v: Any, default: bool = False) -> bool:
    """Coerce a frontmatter value to bool, tolerating yaml-ish strings."""
...
class Skill:
    ...
    body_extra: str = ""
    # Claude-superset execution/invocation manifest fields
    allowed_tools: List[str] = field(default_factory=list)   # tools a bundled script may use
    license: str = ""
    disable_model_invocation: bool = False                   # true = slash/manual only, never auto-injected
    user_invocable: bool = True                              # false = model-only, hidden from slash/menu
    trusted_exec: bool = False                               # imported skills run scripts only once this is set
    ...
    def to_frontmatter(self) -> Dict[str, Any]:
        ...
        if self.allowed_tools:            fm["allowed_tools"] = list(self.allowed_tools)
        if self.license:                  fm["license"] = self.license
        if self.disable_model_invocation: fm["disable_model_invocation"] = True
        if not self.user_invocable:       fm["user_invocable"] = False
        if self.trusted_exec:             fm["trusted_exec"] = True
    ...
    def to_dict(self) -> Dict[str, Any]:
        d = {
            ...
            "disable_model_invocation": bool(self.disable_model_invocation),
            "user_invocable": bool(self.user_invocable),
            "trusted_exec": bool(self.trusted_exec),
    ...
    def from_markdown(cls, text: str, *, path: Optional[str] = None) -> "Skill":
        ...
        return cls(
            ...
            allowed_tools=_as_list(fm.get("allowed_tools")),
            license=str(fm.get("license", "") or ""),
            disable_model_invocation=_as_bool(fm.get("disable_model_invocation"), False),
            user_invocable=_as_bool(fm.get("user_invocable"), True),
            trusted_exec=_as_bool(fm.get("trusted_exec"), False),
            path=path,
        )
    ...
    def validate(self) -> List[str]:
        """Return a list of spec violations (empty == valid). Mirrors Claude's
        SKILL.md rules so imported/authored skills trigger reliably. Advisory:
        callers decide whether to warn or block."""
        issues: List[str] = []
        n = self.name or ""
        if not (1 <= len(n) <= 64):
            issues.append("name must be 1–64 characters")
        if not re.match(r"^[a-z0-9]+(?:-[a-z0-9]+)*$", n):
            issues.append("name must be lowercase kebab-case (no leading/trailing/double hyphen)")
        if n in ("anthropic", "claude"):
            issues.append("name 'anthropic'/'claude' is reserved")
        if not (self.description or "").strip():
            issues.append("description is required (it's what the model matches on)")
