...
class SkillsManager:
    ...
    # ----------------------------------------------------------------------
    # Bundled files (scripts/ references/ assets/) — write / list / delete,
    # plus per-skill import (zip) and export (zip). The portability + authoring
    # half of Claude-style skills.
    # ----------------------------------------------------------------------

    def _find_skill_dir(self, name: str, owner: Optional[str] = None) -> Optional[str]:
        for path in self._iter_skill_files():
            sk = self._read_skill(path)
            if not sk or sk.name != name:
                continue
    ...
    def _safe_join(self, base: str, rel_path: str) -> Optional[str]:
        """Resolve rel_path under base; refuse traversal and the dir root itself."""
        base_r = os.path.realpath(base)
        target = os.path.realpath(os.path.join(base, rel_path))
        try:
            if os.path.commonpath([base_r, target]) != base_r or target == base_r:
                return None
        except ValueError:
            return None
        return target

    def list_skill_files(self, name: str, owner: Optional[str] = None) -> List[Dict]:
        d = self._find_skill_dir(name, owner)
        if not d:
            return []
        base_r = os.path.realpath(d)
        out: List[Dict] = []
        for root, _dirs, files in os.walk(d, followlinks=False):
            for f in files:
                # The SKILL.md at the dir root is edited via its own endpoint.
                if f == "SKILL.md" and os.path.realpath(root) == base_r:
                    continue
    ...
    def write_skill_file(self, name: str, rel_path: str, content,
                         owner: Optional[str] = None, *, binary: bool = False) -> Optional[str]:
        d = self._find_skill_dir(name, owner)
        if not d:
            return None
        rel_path = (rel_path or "").strip().lstrip("/")
        # SKILL.md is edited through the markdown endpoint, not here.
        if rel_path == "SKILL.md":
            return None
        target = self._safe_join(d, rel_path)
        if not target:
            return None
    ...
    def delete_skill_file(self, name: str, rel_path: str, owner: Optional[str] = None) -> bool:
        d = self._find_skill_dir(name, owner)
        if not d:
            return False
        target = self._safe_join(d, (rel_path or "").strip().lstrip("/"))
    ...
    def export_skill_zip(self, name: str, owner: Optional[str] = None) -> Optional[bytes]:
        import io
        import zipfile
        d = self._find_skill_dir(name, owner)
        if not d:
    ...
    def import_skill_zip(self, data: bytes, owner: Optional[str] = None,
                         *, default_category: str = "imported") -> Dict:
        """Unpack a skill zip (SKILL.md + scripts/references/assets). Validates the
        manifest, refuses zip-slip, and stamps owner + source='imported' +
        trusted_exec=False so bundled scripts stay QUARANTINED until trusted."""
        import io
        import zipfile
        try:
        ...
        for n in members:
            ...
            if parts[-1] == "SKILL.md" and len(parts) <= 2:
                ...
                break
        if not md_member:
            return {"ok": False, "error": "no SKILL.md at the zip root (or one folder deep)"}
        prefix = md_member[: -len("SKILL.md")]   # '' or 'folder/'
        try:
            sk = Skill.from_markdown(zf.read(md_member).decode("utf-8"))
        except Exception as e:
            return {"ok": False, "error": f"could not read SKILL.md: {e}"}
        issues = sk.validate()
        if issues:
            return {"ok": False, "error": "invalid SKILL.md: " + "; ".join(issues)}
        sk.owner = owner
        sk.source = "imported"
        sk.trusted_exec = False
        if not sk.category or sk.category == "general":
            sk.category = default_category
        if self._find_skill_dir(sk.name, owner):
            return {"ok": False, "error": f"a skill named {sk.name!r} already exists"}
        dest_dir = self._skill_dir(sk.category, sk.name)
        os.makedirs(dest_dir, exist_ok=True)
        dest_r = os.path.realpath(dest_dir)
        from core.atomic_io import atomic_write_text
        atomic_write_text(os.path.join(dest_dir, "SKILL.md"), sk.to_markdown())
        extracted = 0
        for n in members:
            ...
            try:
                if os.path.commonpath([dest_r, target]) != dest_r:
            ...
            try:
                ...
                continue
        return {"ok": True, "name": sk.name, "category": sk.category,
                "files": extracted, "quarantined": True}
        ...
        for s in self.load(owner=owner):
            ...
            if status not in ("published", None):
                if status == "draft" and s.get("source") == "teacher-escalation":
                    ...
                    continue
            # Manual-only skills (disable_model_invocation) are never auto-listed
            # to the model — they are slash/explicit-invocation only.
            ...
            out.append({
                "name": s["name"],
                # Cap each entry so a verbose description can't blow the
                # Level-0 index budget (mirrors Claude's per-entry cap).
                "description": (s.get("description") or s.get("title", ""))[:200],
                "category": s.get("category", "general"),
                "status": status or "published",
            })
        out.sort(key=lambda x: (x["category"], x["name"]))
        # Bound the index so a large library can't dominate the prompt every turn.
        return out[:80]
    ...
    def get_relevant_skills(
        ...
        # Manual-only skills (disable_model_invocation) never auto-fire via
        # relevance injection — they're reachable only by slash/explicit call.
