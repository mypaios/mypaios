...
class STTService:
    ...
    def transcribe_segments(self, file_path: str, language: str = "") -> list:
        """Timestamped transcription of an audio OR VIDEO file on disk.

        faster-whisper decodes via PyAV, which extracts the audio track
        straight from an MP4/MOV — no demux step needed. Returns
        [{"start": s, "end": s, "text": str}] (empty list on failure).
        Runs on CPU (int8) so it can overlap GPU vision work.
        """
        model = self._get_whisper()
        if not model:
            return []
        try:
            kwargs = {"vad_filter": True}  # avoid hallucination on silence/music
            if language:
                kwargs["language"] = language
            segments, info = model.transcribe(file_path, **kwargs)
            out = [{"start": round(seg.start, 2), "end": round(seg.end, 2),
                    "text": seg.text.strip()} for seg in segments]
            logger.info(f"STT segments: {len(out)} from {file_path} (lang={info.language})")
            return out
        except Exception as e:
