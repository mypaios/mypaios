...
_ROUTING_PATTERNS: tuple[tuple[str, str, Pattern[str]], ...] = tuple(
    ...
    for category, reason, pattern in (
        ...
        # AI image generation (text-to-image). Distinct verbs + image nouns so it
        # won't catch "make a note" or analysis like "what's in this image".
        ("image", "image generation imperative", rf"{_PLEASE}(?:generate|create|make|draw|paint|render|design|produce|whip\s+up|cook\s+up)\s+(?:me\s+)?(?:an?\s+|some\s+|a\s+couple\s+of\s+|\d+\s+)?(?:image|images|picture|pictures|photo|photos|photograph|illustration|drawing|artwork|art|logo|wallpaper|poster|painting|portrait|sketch|render|icon|avatar|banner)\b"),
        ("image", "assistant image generation request", rf"{_ACTION_QUESTION}(?:generate|create|make|draw|paint|render|design|produce)\s+(?:me\s+)?(?:an?\s+|some\s+)?(?:image|images|picture|pictures|photo|photos|photograph|illustration|drawing|artwork|art|logo|wallpaper|poster|painting|portrait|sketch|icon|avatar|banner)\b"),
