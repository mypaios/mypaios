...
async def do_generate_image(content: str, session_id: Optional[str] = None, owner: Optional[str] = None) -> Dict:
    ...
    # Default size from admin settings when the caller didn't specify one (line 3).
    if (len(lines) <= 2 or not lines[2].strip()) and _settings.get("image_size"):
        size = str(_settings["image_size"]).strip()
    ...
    # Local diffusion (FLUX / SDXL / Z-Image) honors an explicit step count from settings.
    if is_local_diffusion:
        try:
            _steps = int(_settings.get("image_steps") or 0)
