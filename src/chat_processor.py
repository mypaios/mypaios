...
class ChatProcessor:
    ...
    # Minimum hybrid score (0.7*cosine + 0.3*keyword) for a RAG result to be
    # injected. 0.35 was too strict for verbose queries against longer note
    # chunks (relevant notes landed ~0.30 and were dropped); 0.25 recalls them
    ...
    def build_context_preface(
        ...
        # PI.md — the user's own first-party instructions (like CLAUDE.md), so
        # injected as TRUSTED system messages right after the preset prompt.
        # (1) Global data/PI.md = user-wide preferences, always on. (2) Project
        # PI.md/AGENTS.md = only when a workspace is active. Global first so a
        # project file can override it in prose.
        try:
            from services.code import repomap as _rm
            _g = _rm.load_global_pi()
            if _g:
                preface.append({"role": "system",
                                "content": "# Your standing preferences (global PI.md — always apply)\n" + _g})
            if workspace:
                _pj = _rm.load_project_pi(workspace)
                if _pj:
                    preface.append({"role": "system",
                                    "content": f"# Project instructions ({_pj[0]}, active workspace)\n{_pj[1].strip()}"})
        except Exception:
            pass
        if not agent_mode:
            try:
