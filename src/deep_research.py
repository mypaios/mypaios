...
Provenance: this engine implements the IterResearch pattern published by
Alibaba-NLP / Tongyi Lab's DeepResearch
(https://github.com/Alibaba-NLP/DeepResearch, Apache-2.0). Upstream Odysseus —
which authored this file, before the MyPaiOS fork point (commit 8354948) —
declared DeepResearch "adapted for [the] Deep Research feature", and that
declaration governs; MyPaiOS did not write the engine and is not in a position
to narrow it. The one piece identified as directly adapted is the goal-based
extraction prompt imported below from src/goal_based_extractor.py, which
carries its own Apache-2.0 §4(b) attribution/change header. Full license text
in licenses/DeepResearch-Apache-2.0.txt; see ACKNOWLEDGMENTS.md.
"""
import asyncio
import json
import logging
...
# Per-mode STRATEGY hints — injected into planning + query generation so each
# mode actually researches differently (not just reformats the final report).
CATEGORY_PLAN_HINTS = {
    "product": "This is PRODUCT research: enumerate the candidate products/options first, then for each plan to gather price, key specs, pros/cons, and independent reviews. Include sub-questions for 'best for <use case>'.",
    "comparison": "This is a COMPARISON: first identify the FULL set of options being compared, then define the shared criteria. Plan a sub-question per criterion so every option is evaluated on the same axes.",
    "howto": "This is a HOW-TO: plan to find the official/authoritative docs and a current, version-correct procedure. Include sub-questions for prerequisites, the exact steps, and common pitfalls/errors.",
    "factcheck": "This is a FACT-CHECK: plan to find PRIMARY sources, and deliberately seek BOTH supporting AND contradicting evidence. Include sub-questions for source credibility and the strongest counter-argument.",
}
CATEGORY_QUERY_HINTS = {
    "product": "Favor queries that surface specific products, prices, spec sheets, and trustworthy review sites/roundups.",
    "comparison": "Make sure every option in the comparison gets its own targeted queries across the shared criteria; don't over-research one option and ignore others.",
    "howto": "Favor official documentation, version-specific tutorials, and troubleshooting/error queries over generic blog posts.",
    "factcheck": "Explicitly issue queries for BOTH 'evidence that <claim> is true' AND 'evidence/criticism that <claim> is false', plus primary/original sources.",
}
...
class DeepResearcher:
    ...
    def __init__(
        ...
        # Per-step routing: the FAST workhorse (llm_model) runs the many cheap
        # calls (plan/query/extract/stop); synthesis_model (optionally a stronger
        # reasoner) runs only the few heavy synthesis/report calls. Defaults to
        # the workhorse so a single model is used unless a synth model is set —
        # avoids loading two big models on one GPU unless the user opts in.
        self.synthesis_model = synthesis_model or llm_model
    ...
    async def _llm(self, messages: List[Dict], temperature: float = 0.3,
        ...
        `model` overrides the workhorse model for a single step — used to route
        the few synthesis/report steps to a stronger reasoner while the many
        high-frequency steps (query-gen, per-page extraction, stop-check) run on
        a FAST model. A heavy reasoning model on every extraction call is what
        caused the 1800s timeouts (each call hit the 90s ceiling, then retried)."""
    ...
    async def _create_plan(self, question: str) -> str:
        ...
        _ph = CATEGORY_PLAN_HINTS.get(getattr(self, "category", None) or "", "")
        if _ph:
            prompt += "\n\nMODE GUIDANCE: " + _ph
        try:
    ...
    async def _generate_queries(self, question: str, report: str,
        ...
        _qh = CATEGORY_QUERY_HINTS.get(getattr(self, "category", None) or "", "")
        if _qh:
            round_instruction += " " + _qh
    ...
    async def _synthesize(self, question: str, findings: List[Dict],
        ...
        try:
            return await self._llm(
                ...
                # A slow local reasoner (dense ~15 tok/s writing up to
                # max_report_tokens, possibly with a thinking preamble) cannot
                # finish in 180s — the old cap silently degraded every round to
                # the findings-list fallback (#1551 regression, seen again with
                # qwen3.6:27b). 420s covers a 4K-token write at ~12 tok/s.
                timeout=420,
                model=getattr(self, "synthesis_model", None),  # reserve the strong reasoner for synthesis
            )
        except Exception as e:
    ...
    async def _final_report(self, question: str, report: str) -> str:
        ...
        cat_extra = CATEGORY_PROMPTS.get(getattr(self, "category", None) or "", "")
        if cat_extra:
            prompt += "\n\n" + cat_extra
        ...
        try:
            result = await self._llm(
                ...
                max_tokens=self.max_report_tokens,
                timeout=420,
                model=getattr(self, "synthesis_model", None),  # the final write benefits from the reasoner
            )
            ...
            if len(result.split()) < 400:
                ...
                expanded = await self._llm(
                    ...
                    max_tokens=self.max_report_tokens,
                    timeout=180,
                    model=getattr(self, "synthesis_model", None),
                )
