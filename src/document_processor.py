...
def analyze_image_with_vl_result(image_path: str, prompt: str = None) -> dict:
    """Analyze an image and return both text and the model that produced it.

    `prompt` overrides the default "Describe this image in detail" — used by
    the video analyzer to inject per-frame timestamps + transcript context.
    """
...
def build_user_content(
    ...
    for fid in attachment_ids or []:
        ...
        elif callable(getattr(upload_handler, "is_video_file", None)) and upload_handler.is_video_file(display_name, mime):
            # Video attachment → inject the timestamped digest (visual scenes +
            # transcript) produced by services/video/analyzer. Analysis is
            # auto-kicked at upload time; if it hasn't finished yet, tell the
            # model honestly and make sure a job is running.
            try:
                from services.video import analyzer as va
                # Digest sidecars live under <uploads root>/.video/ — upload_dir
                # here IS the uploads root (same root the routes use).
                digest = va.load_digest(upload_dir, fid)
                if digest:
                    vid_text = (
                        f"\n\n[Attached video: {display_name}] — a full local analysis was run "
                        "(visual keyframes + speech transcript). Use the timestamped digest below "
                        "to answer questions, summarize, or find key moments. Cite timestamps like "
                        "[mm:ss] in your answers.\n\n" + va.digest_to_text(digest)
                    )
                else:
                    st = va.job_status(fid) or {}
                    if st.get("status") in ("queued", "running"):
                        pct = int(100 * float(st.get("progress") or 0))
                        vid_text = (f"\n\n[Attached video: {display_name} — analysis in progress "
                                    f"({st.get('stage','working')}, ~{pct}%). Tell the user the video is "
                                    "still being analyzed locally and to ask again shortly.]")
                    else:
                        va.start_analysis(path, fid, upload_dir)
                        vid_text = (f"\n\n[Attached video: {display_name} — local analysis just started "
                                    "(keyframes + transcript). Tell the user it's processing in the "
                                    "background and to ask again in a few minutes.]")
                if content and content[0]["type"] == "text":
                    content[0]["text"] += vid_text
                else:
                    content.insert(0, {"type": "text", "text": vid_text.lstrip()})
            except Exception as e:
                logger.error(f"Video digest injection failed for {fid}: {e}")
                if content and content[0]["type"] == "text":
                    content[0]["text"] += "\n\n[Video attached but could not be processed]"
                else:
                    content.insert(0, {"type": "text", "text": "[Video attached but could not be processed]"})
