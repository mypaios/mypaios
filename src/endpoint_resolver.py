...
def build_headers(api_key: Optional[str], base: str) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        # OpenRouter app attribution (rankings + per-app analytics). X-Title is
        # the documented header; X-OpenRouter-Title kept one release for
        # back-compat with anything that grew to expect it.
        headers.setdefault("HTTP-Referer", "https://github.com/mypaios/mypaios")
        headers.setdefault("X-Title", "MyPaiOS")
        headers.setdefault("X-OpenRouter-Title", "MyPaiOS")
    return headers
