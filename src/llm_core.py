...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt PAIOS' canonical OpenAI-style messages to native Ollama /api/chat.

    PAIOS carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        # Keep in sync with endpoint_resolver.build_headers — X-Title is the
        # documented attribution header; legacy X-OpenRouter-Title kept one release.
        h.setdefault("HTTP-Referer", "https://github.com/mypaios/mypaios")
        h.setdefault("X-Title", "MyPaiOS")
        h.setdefault("X-OpenRouter-Title", "MyPaiOS")
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip PAIOS-only metadata before sending messages to providers.
                ...
                if line.startswith("data:"):
                    ...
                    try:
                        if data.strip():
                            if data.startswith("{"):
                                j = json.loads(data)
                                # MID-STREAM ERROR GUARD: once HTTP 200 is
                                # committed, OpenRouter (and some gateways)
                                # deliver failures as normal-looking chunks
                                # {"error": {...}} / finish_reason:"error".
                                # Without this check they'd be recorded as
                                # successful (and costed) messages.
                                _err = j.get("error")
                                _fr0 = ((j.get("choices") or [{}])[0] or {}).get("finish_reason")
                                if _err or _fr0 == "error":
                                    _emsg = (_err or {}).get("message") if isinstance(_err, dict) else str(_err or "provider stream error")
                                    logger.warning("mid-stream provider error (%s): %s", provider, str(_emsg)[:200])
                                    yield f'event: error\ndata: {json.dumps({"status": 502, "text": f"Provider error mid-stream: {_emsg}", "raw": json.dumps(_err)[:500] if _err else ""})}\n\n'
                                    return
                                # Groq puts stream usage inside an x_groq
                                # envelope on the final chunk — lift it so the
                                # generic usage path below sees it.
                                if "usage" not in j and isinstance(j.get("x_groq"), dict) and isinstance(j["x_groq"].get("usage"), dict):
                                    j["usage"] = j["x_groq"]["usage"]
                                ...
                                if "usage" in j and not _delta_has_output:
                                    ...
                                    # ── Widened usage (pricing engine inputs) ──
                                    # cached tokens: OpenAI-style nests them under
                                    # prompt_tokens_details (SUBSET of prompt);
                                    # DeepSeek exposes prompt_cache_hit_tokens.
                                    _ptd = u.get("prompt_tokens_details") or {}
                                    _cached = _ptd.get("cached_tokens") or u.get("prompt_cache_hit_tokens") or 0
                                    if _cached:
                                        _usage_data["cached_tokens"] = _cached
                                    _ctd = u.get("completion_tokens_details") or {}
                                    if _ctd.get("reasoning_tokens"):
                                        _usage_data["reasoning_tokens"] = _ctd["reasoning_tokens"]
                                    # OpenRouter reports the EXACT cost (USD) in
                                    # usage.cost on every response — when present
                                    # it is the billing truth (cost_source=reported).
                                    if u.get("cost") is not None:
                                        try:
                                            _usage_data["cost_usd"] = float(u["cost"])
                                            _usage_data["cost_source"] = "reported"
                                        except (TypeError, ValueError):
                                            pass
                                    if j.get("id"):
                                        _usage_data["response_id"] = j["id"]
                                    if j.get("model"):
                                        _usage_data["served_model"] = j["model"]
