...
def extract_pdf_text(file_path: str) -> str:
    ...
    Prefers **Docling** (OCR-capable; handles scanned/image PDFs and preserves
    tables/layout → Markdown) so all PDF types are usable. Falls back to
    **pypdf** (fast, text-layer only) when Docling is unavailable, disabled
    (PAIOS_PDF_ENGINE=pypdf), or yields nothing.
    """
    # 1) Docling — best for all PDF types (incl. scanned via OCR).
    try:
        from src.docling_runtime import convert_to_markdown as _docling_scan
        md = _docling_scan(file_path)
        if md and md.strip():
            return md
    except Exception as e:
        logger.warning(f"Docling scan failed for {file_path}; falling back to pypdf: {e}")

    # 2) pypdf fallback — text-layer only (no OCR).
    try:
