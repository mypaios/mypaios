...
class ResearchHandler:
    ...
    async def _probe_endpoint(endpoint: str, model: str, headers: dict = None):
        ...
        try:
            ...
            await llm_call_async(
                ...
                max_tokens=5,
                headers=headers,
                # A large local model (20GB+ MoE) cold-loads in 20-60s after a
                # server restart; 15s made this probe fail exactly when nothing
                # was wrong. It gates a multi-minute job — waiting out a cold
                # load is the correct trade.
                timeout=120,
                max_retries=2,
            )
    ...
    async def call_research_service(
        ...
        try:
            ...
            # Optional stronger reasoner for the few synthesis/report calls. Left
            # blank by default → research runs entirely on the fast workhorse
            # model (avoids loading two big models on one GPU). Set
            # research_synthesis_model to e.g. deepseek-r1:32b for max quality.
            from src.settings import get_setting as _gs
            _synth_model = (_gs("research_synthesis_model", "") or "").strip() or None
