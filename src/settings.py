...
DEFAULT_SETTINGS = {
    ...
    "image_size": "1024x1024",   # default WxH for local diffusion (FLUX/SDXL/Z-Image)
    "image_steps": 8,            # inference steps (turbo models ~8, flux schnell ~4)
    "ocr_enabled": True,         # Local AI hub: Docling document/OCR engine
    "ocr_provider": "docling",
    "video_model": "",           # Local AI hub: text-to-video (served endpoint)
    "video_provider": "disabled",
    "local_ai_defaults_applied": False,  # one-shot guard for "recommended stack"
    "vision_model": "",
    "vision_enabled": True,
    ...
    "research_model": "",
    "research_synthesis_model": "",  # optional stronger reasoner for synthesis/report only (blank = use research_model)
    ...
    # --- Crew (local multi-agent mesh) ---
    # Master switch + local-first routing policy for the Supervisor/mini-agent
    # mesh (services/crew). Cloud fallback is OFF by default — the crew runs on
    # local Ollama models unless crew_allow_cloud is explicitly enabled.
    "crew_enabled": True,
    "crew_allow_cloud": False,
    "crew_approval_mode": "plan",        # "plan" = show plan for approval before running; "auto" = run immediately
    "crew_max_steps": 5,                 # max worker steps the planner may produce
    "crew_max_rounds_per_agent": 12,     # per-worker agent-loop round cap
    "crew_supervisor_model": "",         # override the planning/synthesis model (blank = auto local-first)
    "crew_coder_model": "",              # override the Coder worker's model (blank = auto-pick a local coding model)
    "crew_cloud_model": "",              # model name used only when crew_allow_cloud and no local resolves
    # --- Telegram gateway (talk to Pi from anywhere; OFF by default) ---
    "telegram_enabled": False,
    "telegram_bot_token": "",            # user pastes their own @BotFather token
    "telegram_allowed_chat_ids": [],     # only these chat ids may talk to Pi
    # --- Mail Guard (local AI email triage across all accounts; OFF by default) ---
    "mailguard_enabled": False,
    "mailguard_interval_min": 15,        # sweep cadence (minutes)
    "mailguard_filtered_folder": "Pi Filtered",  # where marketing mail is FILED (recoverable)
    "mailguard_hard_delete": False,      # True = marketing goes to Trash instead of the folder
    "mailguard_draft_replies": True,     # draft replies for urgent/important into IMAP Drafts
    "teacher_model": "",
...
_PER_USER_KEYS = {
    ...
    "research_endpoint_id", "research_model", "research_synthesis_model",
}
