...
class TaskScheduler:
    ...
    async def _execute_task_locked(self, task_id: str, run_id: str, *, release_executing: bool = True,
        ...
        try:
            ...
            # A MANUAL ▶ run is allowed on a paused task (pause only stops the
            # schedule). The active-only gate still protects queued SCHEDULED
            # fires from executing after the user pauses/deletes mid-queue.
            _runnable = task is not None and (
                task.status == "active" or (manual and task.status == "paused")
            )
            if not _runnable:
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-PAIOS-Origin"] = "paios-ui"
            msg["X-PAIOS-Kind"] = "task"
            msg["X-PAIOS-Ref"] = str(task.id)
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-PAIOS-Origin": "paios-ui",
                "X-PAIOS-Kind": "task",
                "X-PAIOS-Ref": str(task.id),
            },
        }
        if recipient:
    ...
    async def run_task_now(self, task_id: str, *, force: bool = False):
        """Manually trigger a task execution. Manual runs work on PAUSED
        tasks too — pausing only disables the schedule, not the ▶ button."""
        if force:
