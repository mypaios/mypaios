...
# ── Secret stripping for agent subprocess env (bash/python tools) ──────────
# The agent's bash/python tools inherit the server's environment, which can
# carry provider API keys, OAuth tokens, and internal secrets. A prompt-
# injected `env | curl ...` (or any child process phoning home) must not see
# them. Strip anything secret-shaped; the Cowork toolchain vars it actually
# needs (PATH, HOME, VIRTUAL_ENV, PYTHONPATH, TERM, HF_HOME & other non-secret
# HF_* cache paths, LANG/LC_*) don't match the pattern and pass through.
_SECRET_ENV_RE = re.compile(
    r"_KEY|_TOKEN|_SECRET|_PASSWORD|_PASSWD|_CREDENTIALS?|PAIOS_INTERNAL"
)
# Explicit names, belt-and-braces on top of the pattern (most match anyway).
_SECRET_ENV_NAMES = frozenset({
    "OPENAI_API_KEY", "ANTHROPIC_API_KEY", "GEMINI_API_KEY", "GOOGLE_API_KEY",
    "OPENROUTER_API_KEY", "GROQ_API_KEY", "MISTRAL_API_KEY", "COHERE_API_KEY",
    "DEEPSEEK_API_KEY", "XAI_API_KEY", "BRAVE_API_KEY", "TAVILY_API_KEY",
    "SERPER_API_KEY", "HF_TOKEN", "HUGGING_FACE_HUB_TOKEN",
    "HUGGINGFACEHUB_API_TOKEN", "GITHUB_TOKEN", "GH_TOKEN",
    "AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_SESSION_TOKEN",
})
# Never strip these — the verified Cowork toolchain (git / npm / pytest /
# playwright, workspace-venv resolution) depends on them.
_SUBPROC_ENV_KEEP = frozenset({
    "PATH", "HOME", "VIRTUAL_ENV", "PYTHONPATH", "TERM", "COLUMNS", "LINES",
    "TMPDIR", "USER", "SHELL", "LANG",
})


def _strip_secret_env(env: Dict[str, str]) -> Dict[str, str]:
    """Return a copy of ``env`` without secret-bearing variables.

    Keep-list wins first (Cowork toolchain essentials), then any name that is
    explicitly listed or matches the secret-shaped pattern (case-insensitive
    via uppercasing) is dropped. Everything else passes through unchanged.
    """
    clean: Dict[str, str] = {}
    for k, v in env.items():
        ku = k.upper()
        if ku in _SUBPROC_ENV_KEEP:
            clean[k] = v
            continue
        if ku in _SECRET_ENV_NAMES or _SECRET_ENV_RE.search(ku):
            continue
        clean[k] = v
    return clean
...
async def _direct_fallback(
    ...
    # Secrets (API keys, tokens, PAIOS_INTERNAL_*) are stripped from the
    # inherited env — see _strip_secret_env above. The workspace-venv logic
    # below still layers PATH/VIRTUAL_ENV/PYTHONPATH on top unchanged.
    _subproc_env = {
        **_strip_secret_env(dict(os.environ)),
        "TERM": "xterm-256color",
        "COLUMNS": "120",
        "LINES": "40",
        # Cowork/workspace sessions use the REAL home so git / npm / pytest /
        # playwright toolchains find ~/.gitconfig, ~/.npmrc, caches, browsers,
        # etc. Outside a workspace (generic agent) keep the confined sandbox HOME.
        "HOME": (os.path.expanduser("~") if workspace else _AGENT_WORKDIR),
    }

    # Cowork toolchain: in a workspace, guarantee that a coding model's NATURAL
    # commands (`pytest`, `python`, `python3`, `pip`) resolve to a working
    # interpreter — a fresh folder otherwise has no pytest and macOS `python`
    # doesn't exist, which defeats even capable models. Prefer the workspace's
    # OWN virtualenv (so the user's project deps win); fall back to PiAOS's venv
    # (guaranteed to ship pytest + general libs). System PATH is still appended,
    # so node/npm/npx/git remain available for Playwright & friends.
    if workspace:
        _bin_dirs = []
        _bin_name = "Scripts" if os.name == "nt" else "bin"
        for _vn in (".venv", "venv", "env"):
            _cand = os.path.join(workspace, _vn, _bin_name)
            if os.path.isdir(_cand):
                _bin_dirs.append(_cand)
                _subproc_env["VIRTUAL_ENV"] = os.path.join(workspace, _vn)
                break
        _piaos_bin = os.path.dirname(sys.executable or "")
        if _piaos_bin and os.path.isdir(_piaos_bin):
            _bin_dirs.append(_piaos_bin)
        if _bin_dirs:
            _subproc_env["PATH"] = os.pathsep.join(_bin_dirs) + os.pathsep + _subproc_env.get("PATH", "")
        # Put the workspace root on PYTHONPATH so top-level packages import the
        # way developers expect. Bare `pytest` (unlike `python -m pytest`) does
        # NOT add the rootdir to sys.path, so `from mypkg import ...` in a test
        # would raise ModuleNotFoundError. This mirrors a src-on-path / editable
        # install and makes both `pytest` and `python script.py` resolve sibling
        # packages without the model needing conftest.py tricks.
        _existing_pp = _subproc_env.get("PYTHONPATH", "")
        _subproc_env["PYTHONPATH"] = workspace + (os.pathsep + _existing_pp if _existing_pp else "")

    try:
        if tool == "bash":
            # Safety net (not a full sandbox): refuse catastrophic / forbidden
            # commands so an agent can't nuke the disk, escalate, or publish.
            import re as _re
            _DENY = (
                r"rm\s+-[a-z]*r[a-z]*f?[a-z]*\s+(/|~|\$HOME|/\*|\*)(\s|$)",
                r"\bsudo\b", r"\bgit\s+push\b", r"\b(shutdown|reboot|halt|mkfs|fdisk)\b",
                r"\b(curl|wget)\b[^|\n]*\|\s*(sudo\s+)?(ba|z|fi)?sh\b",
                r"\bdd\b[^\n]*\bof=/dev/", r">\s*/dev/(sd|disk|rdisk|nvme)",
                r":\s*\(\s*\)\s*\{", r"\bchmod\s+-R\s+0?777\s+/",
            )
            _hit = next((p for p in _DENY if _re.search(p, content, _re.I)), None)
            if _hit:
                return {"error": "Refused: command matches a destructive/forbidden pattern "
                                 "(disk-wide deletes, sudo, git push, piping the web into a shell, "
                                 "or raw disk writes). Run it manually if you truly intend to.",
                        "exit_code": 126}
            # Run under real bash (not the system /bin/sh, which is dash-like on
            # macOS) so `echo -e`, heredocs, arrays, [[ ]] etc. behave the way
            # coding models expect when they write files / run tests via shell.
            _shell_exe = "/bin/bash" if (os.name != "nt" and os.path.exists("/bin/bash")) else None
        ...
        # ---- Agentic browser (services/browser) — a persistent headless
        # Chromium page shared across calls, so multi-step flows work.
        # Content arrives either as a bare string (url / click target) or
        # JSON args; be lenient like web_fetch above.
        if tool in ("browser_navigate", "browser_read", "browser_click",
                    "browser_fill", "browser_screenshot"):
            from services.browser import tool as browser_tool
            ...
            if tool == "browser_navigate":
                url = str(args.get("url") or "").strip() or raw.split("\n")[0].strip()
                result = await browser_tool.navigate(url)
            elif tool == "browser_read":
                result = await browser_tool.read(
                    selector=str(args.get("selector") or "").strip(),
                    full=bool(args.get("full")),
                )
            elif tool == "browser_click":
                target = str(args.get("target") or "").strip() or raw.split("\n")[0].strip()
                result = await browser_tool.click(target)
            elif tool == "browser_fill":
                result = await browser_tool.fill(
                    selector=str(args.get("selector") or "").strip(),
                    value=str(args.get("value") or ""),
                    submit=bool(args.get("submit")),
                )
            else:  # browser_screenshot
                result = await browser_tool.screenshot(workspace=workspace)
            _promote_image_fields(result)
            return result
...
async def execute_tool_block(
    ...
    elif tool in ("browser_navigate", "browser_read", "browser_click",
                  "browser_fill", "browser_screenshot"):
        # Agentic browser — persistent headless Chromium (services/browser).
        first_line = content.split(chr(10))[0][:80]
