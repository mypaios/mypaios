...
async def do_manage_skills(content: str, owner: Optional[str] = None) -> Dict:
    ...
    if action in ("run_script", "run", "list_scripts"):
        if not name:
            return {"error": "name is required for run_script", "exit_code": 1}
        # Locate the skill on disk (owner-scoped) to get its directory.
        match = next((s for s in sm.load(owner=owner) if s.get("name") == name), None)
        if not match:
            return {"error": f"Skill {name!r} not found", "exit_code": 1}
        skill_path = match.get("path")
        if not skill_path:
            return {"error": f"Skill {name!r} has no directory on disk", "exit_code": 1}
        skill_dir = os.path.dirname(skill_path)
        from services.memory.skill_runner import run_skill_script, list_skill_scripts

        if action == "list_scripts":
            scripts = list_skill_scripts(skill_dir)
            return {"results": ("\n".join(scripts) if scripts else "(no runnable scripts in scripts/)")}

        # Quarantine: an IMPORTED skill cannot run bundled code until the owner
        # reviews it and sets trusted_exec. (Owner-authored skills run freely —
        # same trust boundary as the bash tool the owner already has.)
        if (match.get("source") == "imported") and not match.get("trusted_exec"):
            return {
                "error": (f"Skill {name!r} was imported and is quarantined. Review its SKILL.md + "
                          f"scripts/, then set trusted_exec: true in the frontmatter to allow run_script."),
                "exit_code": 1,
            }
        script = (args.get("script") or args.get("path") or "").strip()
        if not script:
            return {"error": "script is required for run_script (e.g. 'build.py'). Use action='list_scripts' to see options.", "exit_code": 1}
        run_args = args.get("args") or []
        if isinstance(run_args, str):
            run_args = [run_args]
        res = run_skill_script(skill_dir, script, list(run_args))
        if res.get("error"):
            return {"error": res["error"], "exit_code": res.get("exit_code", 1)}
        body = res.get("stdout") or ""
        if res.get("stderr"):
            body += ("\n\n[stderr]\n" + res["stderr"])
        return {"results": (body.strip() or "(script produced no output)"), "exit_code": res.get("exit_code", 0)}

    if action in ("write_file", "add_file"):
        if not name:
            return {"error": "name is required for write_file", "exit_code": 1}
        rel = (args.get("path") or args.get("file") or "").strip()
        if not rel:
            return {"error": "path is required for write_file (e.g. 'scripts/build.py' or 'references/spec.md')", "exit_code": 1}
        content = args.get("content")
        if content is None:
            return {"error": "content is required for write_file", "exit_code": 1}
        written = sm.write_skill_file(name, rel, content, owner=owner)
        if written is None:
            return {"error": f"Could not write {rel!r} (skill {name!r} not found, or unsafe path).", "exit_code": 1}
        return {"results": f"Wrote {written} to skill {name!r}."}

    if action == "list_files":
        if not name:
            return {"error": "name is required for list_files", "exit_code": 1}
        files = sm.list_skill_files(name, owner=owner)
        if not files:
            return {"results": "(no bundled files — add one with action='write_file')"}
        return {"results": "\n".join(f"{f['path']} ({f['size']} bytes)" for f in files)}

    if action == "delete_file":
        if not name:
            return {"error": "name is required for delete_file", "exit_code": 1}
        rel = (args.get("path") or "").strip()
        if not rel:
            return {"error": "path is required for delete_file", "exit_code": 1}
        ok = sm.delete_skill_file(name, rel, owner=owner)
        return {"results": f"Deleted {rel}."} if ok else {"error": f"Could not delete {rel!r}.", "exit_code": 1}
...
async def do_app_api(content: str, owner: Optional[str] = None) -> Dict:
    """Generic loopback to any internal PAIOS API endpoint. Lets the
    ...
    for t in merged:
        ...
        if t.get("status") == "error" and t.get("output_tail"):
            ...
            if tail:
                ...
                # banner ("Locale: C / Ubuntu_PAIOS ❯") — useless for
...
async def do_tail_serve_output(content: str, owner: Optional[str] = None) -> Dict:
            ...
            for t in (state.get("tasks") or []):
                if isinstance(t, dict) and (t.get("sessionId") == session_id or t.get("id") == session_id):
                    ...
                    if not sport:
                        sport = t.get("sshPort") or ""
                    break
    # Prefer the persisted /tmp/paios-tmux/SESSION.log file over the
    ...
    log_path = f"/tmp/paios-tmux/{session_id}.log"
