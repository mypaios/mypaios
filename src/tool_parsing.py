...
def parse_tool_blocks(text: str) -> List[ToolBlock]:
    ...
    # Pattern 5: NAKED function-call JSON. Smaller models (hermes3:8b in
    # scheduled agent runs, verified 2026-06-10) sometimes emit the tool call
    # as plain text — `{"function": "ls", "arguments": {"path": "~"}}` — with
    # no fence, no XML, no native tool_calls. Without this rescue the round
    # parses as "no tools" and the JSON blob becomes the final answer.
    # Guarded tightly so a legitimate JSON *answer* is never hijacked: the
    # ENTIRE response (or its single ```json fence) must be one object whose
    # only keys are a name + arguments pair, and the name must be a real tool.
    if not blocks:
        block = _parse_naked_function_json(text)
        if block:
            blocks.append(block)
...
_NAKED_KEYS_NAME = ("function", "name", "tool")
_NAKED_KEYS_ARGS = ("arguments", "parameters", "args")


def _parse_naked_function_json(text: str) -> Optional[ToolBlock]:
    raw = (text or "").strip()
    ...
    if not isinstance(obj, dict):
        return None
    name = next((obj[k] for k in _NAKED_KEYS_NAME if isinstance(obj.get(k), str)), None)
    if not name:
        return None
    extras = set(obj) - set(_NAKED_KEYS_NAME) - set(_NAKED_KEYS_ARGS)
    if extras:  # any other top-level key → it's data, not a call
        return None
    args = next((obj[k] for k in _NAKED_KEYS_ARGS if isinstance(obj.get(k), dict)), {})
    from src.agent_tools import TOOL_TAGS
    if name not in TOOL_TAGS:
        return None
    # Reuse the native-call converter so per-tool content formats stay
    # single-sourced (lazy import — tool_schemas imports this module).
    from src.tool_schemas import function_call_to_tool_block
    try:
        ...
        return None
    if block:
        logger.info(f"[tool-parse] rescued naked function-call JSON -> {name}")
    return block
