...
class UploadHandler:
    ...
    def is_video_file(self, filename: str, content_type: str = None) -> bool:
        """Check if a file is a video file based on extension or content type.

        `.webm` is ambiguous (mic voice notes are audio/webm) — it only counts
        as video when the MIME type says `video/*`, so voice recordings keep
        routing to the audio path.
        """
        ...
        if ext in video_extensions:
            return True
        if ext == '.webm' and content_type and content_type.startswith('video/'):
            return True
        if content_type and content_type in video_mime_types:
            return True
        return False
    ...
    def save_upload(self, u: UploadFile, client_ip: str, owner: str = None) -> dict:
        ...
        # Videos get a larger cap (meeting recordings are hundreds of MB);
        # everything else keeps the tight default.
        _size_cap = self.max_upload_size
        if self.is_video_file(getattr(u, "filename", "") or "", getattr(u, "content_type", None)):
            _size_cap = 512 * 1024 * 1024
        if file_size > _size_cap:
            raise HTTPException(
                status_code=400,
                detail=f"File size exceeds {_size_cap/1024/1024:.0f}MB limit"
            )
