#!/bin/bash
# PAIOS — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything PAIOS needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine PAIOS
...
# Shell overrides (PAIOS_PORT / PAIOS_HOST) take top priority, then .env
# values (APP_PORT / APP_BIND), then built-in defaults.
PORT="${PAIOS_PORT:-${APP_PORT:-7860}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
HOST="${PAIOS_HOST:-${APP_BIND:-127.0.0.1}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
echo "▶ PAIOS quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
  ...
  echo "    PAIOS_PORT=7900 ./start-macos.sh"
  exit 1
fi
...
echo "▶ Preparing PAIOS…"
PAIOS_SKIP_RUN_HINT=1 ./venv/bin/python setup.py
...
# PAIOS_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$PAIOS_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
  (
    for _ in $(seq 1 90); do
      if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
        printf '\n'
        printf '  ┌────────────────────────────────────────────┐\n'
        printf '  │  ✓ PAIOS is ready — opening your browser  │\n'
        printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting PAIOS — it will open in your browser at $URL"
