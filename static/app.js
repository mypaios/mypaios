// ============================================
// MyPaiOS UI — Main Application Orchestrator
...
import localAIModule from './js/localAI.js';
import coworkModule from './js/cowork.js';
import crewModule from './js/crew.js';
import codeModule from './js/code.js';
import activityModule from './js/activity.js';
import storagePanelModule from './js/storagePanel.js';
import videoHubModule from './js/videoHub.js';
// Artifacts — live preview of generated HTML/SVG/React (chip on code blocks).
import artifactsModule from './js/artifacts.js';
// Voice conversation mode — hands-free talk loop (STT → chat → TTS).
import voiceModeModule from './js/voiceMode.js';
// Mail Guard — local AI email triage panel (shield in the Email header).
import mailGuardModule from './js/mailGuard.js';
import adminModule from './js/admin.js';
import settingsModule from './js/settings.js';
// Eagerly bind unified minimize/restore behavior across all tool modals.
import './js/modalManager.js';
// Single-active "one screen at a time" coordinator — closes peer tools on open,
// frames the active tool as a full chat-area screen, owns tool z-ordering.
import './js/panelCoordinator.js';
...
// Help guide — opens the /help page in a full-height in-app iframe modal
let _helpOpen = false;
function _openHelpPanel(hash) {
  if (_helpOpen) return;
  _helpOpen = true;
  const deep = (typeof hash === 'string' && hash.startsWith('#')) ? hash : '';
  const titleTxt = deep === '#whats-new' ? '🆕 What’s New' : '🛸 MyPaiOS Help Guide';
  const modal = document.createElement('div');
  modal.className = 'modal';
  modal.id = 'help-modal';
  modal.innerHTML = `
    <div class="modal-content" style="max-width:1100px;width:97vw;height:90vh;display:flex;flex-direction:column;padding:0;overflow:hidden;">
      <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;border-bottom:1px solid var(--border);flex-shrink:0;">
        <span style="font-weight:600;flex:1;font-size:14px;">${titleTxt}</span>
        <span id="help-version" style="font-size:11px;opacity:.55;padding:2px 8px;border:1px solid var(--border);border-radius:12px;"></span>
        <button class="close-btn" id="help-close">✖</button>
      </div>
      <iframe src="/help${deep}" style="flex:1;border:none;width:100%;background:var(--bg);" title="MyPaiOS Help Guide"></iframe>
    </div>`;
  document.body.appendChild(modal);
  modal.querySelector('#help-close').addEventListener('click', _closeHelpPanel);
  modal.addEventListener('mousedown', (e) => { if (e.target === modal) _closeHelpPanel(); });
  document.addEventListener('keydown', _helpEsc);
  fetch('/api/version').then(r => r.json()).then(d => {
    const v = modal.querySelector('#help-version');
    if (v && d.version) v.textContent = 'v' + d.version;
  }).catch(() => {});
}
function _closeHelpPanel() {
  _helpOpen = false;
  document.removeEventListener('keydown', _helpEsc);
  const m = document.getElementById('help-modal');
  if (m) m.remove();
}
function _helpEsc(e) { if (e.key === 'Escape' && _helpOpen) _closeHelpPanel(); }
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__paiosDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  // Content growth (streaming, images finishing) doesn't fire scroll events —
  // re-check the arrow whenever messages change.
  new MutationObserver(uiModule.debounce(_updateScrollBtn, 150))
    .observe(el('chat-history'), { childList: true, subtree: true });
  el('scroll-bottom-btn')?.addEventListener('click', () => {
    uiModule.setAutoScroll(true);
    uiModule.scrollHistoryJumpToBottom();
  });
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'MyPaiOS Chat';
  ...
  if (toolCompareBtn) {
    toolCompareBtn.addEventListener('click', () => {
      if (window.expandSection) window.expandSection('tools-section');
      if (compareModule) {
  ...
  if (toolCookbookBtn) {
    toolCookbookBtn.addEventListener('click', async () => {
      if (window.expandSection) window.expandSection('system-section');
  ...
  if (toolDoclibBtn) {
    toolDoclibBtn.addEventListener('click', () => {
      if (window.expandSection) window.expandSection('knowledge-section');
  ...
  if (toolTasksBtn) {
  // Agents (chat-managed agent hub) — sidebar + rail
  const agentsBtns = [el("rail-agents"), el("tool-agents-btn")].filter(Boolean);
  agentsBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      if (window.expandSection) window.expandSection('automate-section');
      if (agentsHubModule) agentsHubModule.togglePanel();
    });
  });

  // Files (drive sync & name search — the former Agents panel)
  const filesBtn = el("tool-files-btn");
  if (filesBtn) filesBtn.addEventListener("click", () => {
    if (window.expandSection) window.expandSection('knowledge-section');
    if (agentsModule) agentsModule.togglePanel();
  });

  // Local AI hub button (sidebar + rail)
  const localAIBtns = [el("rail-local-ai"), el("tool-local-ai-btn")].filter(Boolean);
  localAIBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      if (window.expandSection) window.expandSection('system-section');
      if (localAIModule) localAIModule.togglePanel();
    });
  });

  // Cowork (local coding agent) button
  const coworkBtns = [el("rail-cowork"), el("tool-cowork-btn")].filter(Boolean);
  coworkBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      if (window.expandSection) window.expandSection('code-section');
      if (coworkModule) coworkModule.togglePanel();
    });
  });

  // Crew (local multi-agent mesh) button
  const crewBtns = [el("rail-crew"), el("tool-crew-btn")].filter(Boolean);
  crewBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      if (window.expandSection) window.expandSection('code-section');
      if (crewModule) crewModule.togglePanel();
    });
  });

  // Code (project-aware coding studio) button
  const codeBtns = [el("rail-code"), el("tool-code-btn")].filter(Boolean);
  codeBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      if (window.expandSection) window.expandSection('code-section');
      if (codeModule) codeModule.togglePanel();
    });
  });

  // Activity / Capacity button + live badge monitor
  const activityBtns = [el("rail-activity"), el("tool-activity-btn")].filter(Boolean);
  activityBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      if (window.expandSection) window.expandSection('system-section');
      if (activityModule) activityModule.togglePanel();
    });
  });
  if (activityModule) { try { activityModule.startMonitor(); } catch (e) {} }

  // Artifacts — watch rendered markdown for previewable HTML/SVG/React blocks
  if (artifactsModule) { try { artifactsModule.init(); } catch (e) {} }

  // Voice conversation mode — composer waveform button toggles the talk loop
  const voiceModeBtn = el("voice-mode-btn");
  if (voiceModeBtn) voiceModeBtn.addEventListener("click", () => {
    if (voiceModeModule) voiceModeModule.start();
  });

  // Mail Guard — shield button in the Email section header
  const mailGuardBtn = el("email-guard-btn");
  if (mailGuardBtn) mailGuardBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    if (mailGuardModule) mailGuardModule.togglePanel();
  });

  // Storage — disks, model storage, Pi footprint
  const storageBtn = el("tool-storage-btn");
  if (storageBtn) storageBtn.addEventListener("click", () => {
    if (window.expandSection) window.expandSection('system-section');
    if (storagePanelModule) storagePanelModule.togglePanel();
  });

  // Video — local video analysis (upload, digest, ask in chat)
  const videoBtn = el("tool-video-btn");
  if (videoBtn) videoBtn.addEventListener("click", () => {
    if (window.expandSection) window.expandSection('knowledge-section');
    if (videoHubModule) videoHubModule.togglePanel();
  });

  // Add API provider — jump straight to Settings → Services → Add API
  // (OpenRouter / DeepSeek / OpenAI / Anthropic / Groq / Mistral / Gemini / xAI…)
  const addApiBtn = el("add-api-provider-btn");
  if (addApiBtn) addApiBtn.addEventListener("click", () => {
    if (settingsModule && settingsModule.open) settingsModule.open("services");
    setTimeout(() => {
      const f = el("adm-add-api");
      if (f) f.scrollIntoView({ behavior: "smooth", block: "start" });
    }, 250);
  });

  // Help — open the guide in an in-app modal (same-origin iframe)
  const helpBtn = el("tool-help-btn");
  if (helpBtn) helpBtn.addEventListener("click", () => {
    if (window.expandSection) window.expandSection('app-section');
    _openHelpPanel();
  });

  // Version — opens the guide deep-linked to the What's New changelog
  const versionBtn = el("tool-version-btn");
  if (versionBtn) versionBtn.addEventListener("click", () => {
    if (window.expandSection) window.expandSection('app-section');
    _openHelpPanel('#whats-new');
  });
  fetch('/api/version').then(r => r.json()).then(d => {
    const b = el('tool-version-badge');
  ...
  const _routeOpen = {
    ...
    '/email':    () => {
      ...
      const hdr = document.querySelector('#email-section .section-header-flex') || document.getElementById('email-section');
      if (hdr) hdr.click();
  ...
  if (_opener) window._paiosRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('paios-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'paios-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Pi…');
      }
  ...
  const UI_VIS_KEY = 'paios-ui-visibility';
  ...
  const UI_VIS_DEFAULT_OFF = new Set(['rag-toggle-btn']);
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'paios-toolbar-visibility';
    try {
  ...
  // Elastic overscroll: REMOVED. The custom translateY rubber-band made the
  // whole chat jiggle on every wheel tick at the edges — and when the history
  // didn't overflow (short/empty chat), scrollHeight == clientHeight meant
  // EVERY wheel event was "at an edge", so all scrolling became pure bounce
  // ("UI not stable / non-scrollable"). macOS provides native rubber-banding;
  // never reintroduce a transform-based one here.
  ...
  // Close chat (X) click -> clear session and show welcome screen
  const chatCloseBtn = el('chat-close-btn');
  if (chatCloseBtn) {
    chatCloseBtn.addEventListener('click', () => {
      ...
      // Clear chat history content
      const box = el('chat-history');
      if (box) box.innerHTML = '';
      // Show welcome screen
      if (chatModule && chatModule.showWelcomeScreen) chatModule.showWelcomeScreen();
      // Remove active session states in sidebar
      document.querySelectorAll('.session-item.active').forEach(s => s.classList.remove('active'));
      document.querySelectorAll('.list-item.active-session').forEach(s => s.classList.remove('active-session'));
      // Reset meta text
      const currentMetaEl = el('current-meta');
      if (currentMetaEl) currentMetaEl.textContent = 'MyPaiOS Chat';
      // Hide close button
...
// ============================================
function startPAIOSApp() {
  if (window.__paiosAppStarted) return;
  window.__paiosAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('paios-doc-open-' + _curSession) === '1') {
  ...
  window._paiosBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (messageInput) {
    messageInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' && !e.shiftKey && !e.isComposing) {
        ...
        // Mid-stream Enter would be silently swallowed downstream — tell the
        // user what to do instead of looking dead.
        if (sendBtn && sendBtn.dataset.mode === 'streaming') {
          try { uiModule.showToast('Still responding — press Stop ■ to interrupt'); } catch {}
          return;
        }
        handleSubmit(e);
      }
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._paiosRouteOpener) {
          try { window._paiosRouteOpener(); } catch (_) {}
          window._paiosRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startPAIOSApp, { once: true });
} else {
  startPAIOSApp();
}
