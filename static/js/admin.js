...
async function _selectAddedModelInChat(endpoint) {
  ...
  try {
    document.dispatchEvent(new CustomEvent('paios:auto-select-model', {
      detail: {
...
function initEndpointForm() {
  ...
  // Enter-to-submit: pressing Enter in any Add-Models field clicks its Add
  // button (you no longer have to mouse over after pasting a URL/key).
  [['adm-epUrl', 'adm-epAddBtn'], ['adm-epApiKey', 'adm-epAddBtn'],
   ['adm-epLocalUrl', 'adm-epLocalAddBtn'], ['adm-epLocalApiKey', 'adm-epLocalAddBtn']]
    .forEach(([inId, btnId]) => {
      const ip = el(inId);
      if (ip) ip.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') { e.preventDefault(); const b = el(btnId); if (b) b.click(); }
      });
    });
  ...
  document.querySelectorAll('#adm-add-api, #adm-add-local').forEach((sec) => {
    ...
    if (!head) return;
    const key = 'paios.addModels.' + sec.id + '.open';
    let open = false;
...
function initBackup() {
  el('adm-exportDataBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const filename = match ? match[1] : 'paios_backup.json';
