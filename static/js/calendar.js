...
const BADGE_SEEN_KEY = 'paios-calendar-badge-seen';
...
function _wireAll(body) {
  ...
  try {
    ...
    if (calBody && splitter) {
      ...
      if (!alreadySet) {
        const saved = parseInt(localStorage.getItem('paios.cal.detailH') || '0', 10);
      ...
      const onUp = () => {
        ...
        if (px) { try { localStorage.setItem('paios.cal.detailH', String(px)); } catch {} }
      };
      const onDown = (ev) => {
        ev.preventDefault();
        dragging = true;
      ...
      const resetSplit = () => {
        ...
        try { localStorage.removeItem('paios.cal.detailH'); } catch {}
      };
...
const LS_KEY = 'paios-calendar-cache';
