...
// One consistent 12px stroke-icon set for every message action. The old mix
// of unicode glyphs (✎ ↻ ✂ ? ⨝ ✕) rendered at different sizes/baselines per
// font and needed per-glyph pixel nudges in CSS; the ✕ delete in particular
// read as a "close window" control.
const _msgIcon = (paths) => `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${paths}</svg>`;
const EDIT_ICON = _msgIcon('<path d="M17 3a2.83 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/>');
const REGEN_ICON = _msgIcon('<path d="M1 4v6h6"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/>');
const SHORTEN_ICON = _msgIcon('<circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><line x1="20" y1="4" x2="8.12" y2="15.88"/><line x1="14.47" y1="14.48" x2="20" y2="20"/><line x1="8.12" y1="8.12" x2="12" y2="12"/>');
const EXPLAIN_ICON = _msgIcon('<circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/>');
const FORK_ICON = _msgIcon('<line x1="6" y1="3" x2="6" y2="15"/><circle cx="18" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M18 9a9 9 0 0 1-9 9"/>');
const DELETE_ICON = _msgIcon('<polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>');
const RESEND_ICON = _msgIcon('<line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/>');
const DOWNLOAD_ICON = _msgIcon('<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/>');
...
export function buildImageBubble(imageUrl, prompt, model, size, quality, imageId) {
  ...
  dlBtn.addEventListener('click', async (e) => {
    ...
    } catch { dlBtn.textContent = '\u2717'; setTimeout(() => { dlBtn.innerHTML = DOWNLOAD_ICON; }, 1500); }
  });
...
// Every message shows the SAME two actions in the same order, rest under ···.
// (This used to promote the 2 most-recently-used actions from localStorage,
// which made the row differ from message to message — and pinned the ✕
// delete onto every bubble once it had been used, where it read as a
// "close" button. Consistency beats adaptivity here; never reintroduce.)
...
export function createMsgFooter(msgElement) {
  ...
  // All actions, in fixed display order \u2014 first _MAX_VISIBLE are always
  // visible, the rest go under \u00B7\u00B7\u00B7. Delete is deliberately last + danger.
  const allActions = [
    { id: 'copy', icon: COPY_ICON, title: 'Copy message', cls: 'footer-copy-btn', handler(e) {
      e.stopPropagation();
    ...
    { id: 'regen', icon: REGEN_ICON, title: 'Regenerate from here', cls: 'msg-action-btn', handler(e) {
      e.stopPropagation();
      if (window.chatModule?.regenerateFrom) window.chatModule.regenerateFrom(msgElement);
    }},
    { id: 'edit', icon: EDIT_ICON, title: 'Edit', cls: 'msg-action-btn', handler(e) {
      e.stopPropagation();
      if (window.chatModule?.editAIMessage) window.chatModule.editAIMessage(msgElement);
    }},
    { id: 'shorten', icon: SHORTEN_ICON, title: 'Rewrite shorter', cls: 'msg-action-btn', handler(e) {
      e.stopPropagation();
    ...
    { id: 'explain', icon: EXPLAIN_ICON, title: 'Explain simpler', cls: 'msg-action-btn', handler(e) {
      e.stopPropagation();
    ...
    { id: 'fork', icon: FORK_ICON, title: 'Fork conversation', cls: 'msg-action-btn', handler(e) {
      e.stopPropagation();
      if (window.chatModule?.forkFrom) window.chatModule.forkFrom(msgElement);
    }},
    { id: 'delete', icon: DELETE_ICON, title: 'Delete message', cls: 'msg-action-btn msg-delete-btn', danger: true, handler(e) {
      e.stopPropagation();
      if (window.chatModule?.deleteMessage) window.chatModule.deleteMessage(msgElement);
    }},
  ];

  const visible = allActions.slice(0, _MAX_VISIBLE);
  const overflow = allActions.slice(_MAX_VISIBLE);

  // Render visible buttons
  function _addBtn(action, container) {
    const btn = _makeActionBtn(action.cls, action.title, '', action.handler);
  ...
  if (overflow.length > 0) {
    ...
    moreBtn.addEventListener('click', (e) => {
      ...
      overflow.forEach(a => {
        ...
        item.className = 'msg-overflow-item' + (a.danger ? ' danger' : '');
        item.type = 'button';
        item.title = a.title;
...
/**
 * Create a footer row for a user message with action buttons (same system as
 * the AI footer: fixed order, first _MAX_VISIBLE visible, delete under \u00B7\u00B7\u00B7).
 */
export function createUserMsgFooter(msgElement) {
  ...
  const allActions = [
    ...
    { id: 'edit', icon: EDIT_ICON, title: 'Edit message', cls: 'msg-action-btn', handler(e) {
      e.stopPropagation();
      if (window.chatModule?.editUserMessage) window.chatModule.editUserMessage(msgElement);
    }},
    { id: 'resend', icon: RESEND_ICON, title: 'Resend message', cls: 'msg-action-btn', handler(e) {
      e.stopPropagation();
  ...
  visible.forEach(a => {
    const btn = _makeActionBtn(a.cls, a.title, '', a.handler);
...
export function addMessage(role, content, modelName, metadata) {
  try {
    if (activityTracker && activityTracker.isEnabled()) {
      activityTracker.log('chat_message', {
        role: role,
        content: typeof content === 'string' ? content : JSON.stringify(content),
        model: modelName || (metadata && metadata.model) || null
      });
    }
    hideWelcomeScreen();
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Pi' : shortModel(resolvedModel);
