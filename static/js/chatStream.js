...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      document.querySelectorAll('.paios-highlight').forEach(function(e) { e.classList.remove('paios-highlight'); });
      document.querySelectorAll('.paios-hl-label').forEach(function(e) { e.remove(); });
      var target = document.querySelector(uiData.selector);
      if (target) {
        target.classList.add('paios-highlight');
        target.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        if (uiData.label) {
          var lbl = document.createElement('div');
          lbl.className = 'paios-hl-label';
