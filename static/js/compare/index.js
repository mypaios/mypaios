...
import {
  ...
  registerPaneActions, applyBlindLabels,
} from './panes.js';
...
async function _buildCompareUI() {
  ...
  // Names/Blind live toggle — flips model-name visibility mid-comparison.
  const EYE_OPEN_I = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
  const EYE_OFF_I = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>';
  const namesBtn = document.createElement('button');
  namesBtn.id = 'compare-names-btn';
  namesBtn.style.cssText = _btnCSS;
  const _renderNamesBtn = () => {
    namesBtn.innerHTML = (state._blindMode ? EYE_OPEN_I : EYE_OFF_I) +
      '<span style="font-size:11px;margin-left:3px;">' + (state._blindMode ? 'Names' : 'Blind') + '</span>';
    namesBtn.title = state._blindMode ? 'Reveal model names' : 'Hide model names (blind testing)';
  };
  _renderNamesBtn();
  namesBtn.addEventListener('click', () => {
    state._blindMode = !state._blindMode;
    try { localStorage.setItem('compare-blind', state._blindMode ? '1' : '0'); } catch (e) {}
    applyBlindLabels();
    buildVoteBar(state._selectedModels.length);
    _renderNamesBtn();
  });
  headerActions.appendChild(namesBtn);
...
function _buildComparisonMarkdown() {
  ...
  panes.forEach((pane, i) => {
    ...
    // The first '.body' in DOM order is the USER bubble — export the last
    // AI message's body instead, else every section repeats the prompt.
    const aiMsgs = pane.querySelectorAll('.msg-ai');
    const aiLast = aiMsgs.length ? aiMsgs[aiMsgs.length - 1] : null;
    const body = aiLast ? aiLast.querySelector('.compare-text-content, .msg-body, .body') || aiLast
                        : pane.querySelector('.compare-text-content');
