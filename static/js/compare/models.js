...
const IMAGE_KEYWORDS = ['dall-e', 'gpt-image', 'chatgpt-image', 'stable-diffusion', 'sdxl', 'flux', 'midjourney', 'z-image'];

function classifyModel(id) {
  const lower = id.toLowerCase();
  if (DEPRECATED_IMAGE.some(p => lower.startsWith(p))) return 'other';
  if (IMAGE_PREFIXES.some(p => lower.startsWith(p)) || IMAGE_KEYWORDS.some(p => lower.includes(p))) return 'image';
...
function _persistSelections() {
  if (state._selectedModels.length > 0) {
    Storage.setJSON('paios-compare-selections-' + (state._compareMode || 'chat'), state._selectedModels);
  }
  if ((state._compareMode === 'search' || state._compareMode === 'research') && state._searchSynthModels) {
    Storage.setJSON('paios-compare-synth-' + state._compareMode, state._searchSynthModels);
  }
}
...
const POOL_STORAGE_KEY = 'paios-shuffle-pool-excluded';
