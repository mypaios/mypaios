...
async function _createAndAppendPane(m) {
  ...
  if (!res.ok) { uiModule.showError('Could not add ' + (m.name || 'model')); return; }
  const data = await res.json();
...
function _removePane(paneIdx) {
  ...
  // Capture surviving panes' rendered content BEFORE re-indexing — the
  // rebuild below recreates empty pane DOM, and without this every other
  // model's response/timer/badge was silently wiped on remove.
  const _kept = [];
  for (let oi = 0; oi < state._selectedModels.length; oi++) {
    if (oi === paneIdx) continue;
    const h = document.getElementById('cmp-history-' + oi);
    const t = document.getElementById('cmp-timer-' + oi);
    const b = document.getElementById('cmp-badge-' + oi);
    _kept.push({ hist: h ? h.innerHTML : '', timer: t ? t.textContent : '',
                 badge: b ? b.innerHTML : '', badgeCls: b ? b.className : '' });
  }
  ...
  // Restore the surviving panes' responses/timers/badges captured above.
  for (let i = 0; i < n && i < _kept.length; i++) {
    const h = document.getElementById('cmp-history-' + i);
    if (h && _kept[i].hist) h.innerHTML = _kept[i].hist;
    const t = document.getElementById('cmp-timer-' + i);
    if (t) t.textContent = _kept[i].timer;
    const b = document.getElementById('cmp-badge-' + i);
    if (b && _kept[i].badge) { b.innerHTML = _kept[i].badge; b.className = _kept[i].badgeCls; }
    if (_kept[i].hist) {
      const paneEl = document.querySelector('.compare-pane[data-pane="' + i + '"]');
      if (paneEl) paneEl.querySelectorAll('.pane-needs-response').forEach((btn) => { btn.style.display = ''; });
    }
  }
  ...
  filtered.forEach(m => {
    ...
    item.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (data && data.id) { state._paneSessionIds[paneIdx] = data.id; swapOk = true; }
        }
      } catch (err) {
        console.error('Failed to create session for swapped model:', err);
      }
      if (!swapOk) { uiModule.showError('Could not switch to ' + (m.name || 'that model')); return; }
...
function shufflePanePositions() {
  ...
  // Collect pane contents (HTML) + timers before swapping
  const paneContents = [];
  const paneClasses = [];
  const paneTimers = [];
  for (let i = 0; i < n; i++) {
    ...
    const timer = document.getElementById('cmp-timer-' + i);
    paneTimers.push(timer ? timer.textContent : '');
  ...
  setTimeout(() => {
    for (let i = 0; i < n; i++) {
      ...
      if (badge) { badge.textContent = ''; badge.style.color = ''; }
      // Carry the elapsed-time readout with its pane content (it was left in
      // place before, so each timer showed a DIFFERENT model's elapsed time).
      const timerEl = document.getElementById('cmp-timer-' + i);
      if (timerEl) timerEl.textContent = paneTimers[src] || '';
      if (pane) {
  ...
  // After shuffle: re-blind only when the user's preference is blind —
  // with names-visible preference a shuffle just reorders panes.
  try { state._blindMode = localStorage.getItem('compare-blind') === '1'; } catch (e) {}
...
/** Re-label every pane title / vote button / message role + the header line
 *  from the CURRENT state._blindMode. Lets the header eye-toggle flip between
 *  "Model A" letters and real model names live, mid-comparison. */
function applyBlindLabels() {
  for (let i = 0; i < state._selectedModels.length; i++) {
    const m = state._selectedModels[i];
    const label = state._blindMode ? 'Model ' + _slotChar(i) : (m.name || m.model);
    const title = document.getElementById('cmp-title-' + i);
    if (title) title.innerHTML = escapeHtml(label) + ' <span class="pane-title-caret">&#x25BE;</span>';
    const pane = document.querySelector('.compare-pane[data-pane="' + i + '"]');
    const voteLbl = pane && pane.querySelector('.pane-vote-label');
    if (voteLbl) voteLbl.textContent = 'Vote ' + label;
    const hist = document.getElementById('cmp-history-' + i);
    if (hist) {
      const aliases = ['AI', 'Model ' + _slotChar(i), m.name, m.model].filter(Boolean);
      hist.querySelectorAll('.msg-ai .role').forEach((r) => {
        if (aliases.indexOf(r.textContent.trim()) !== -1) r.textContent = label;
      });
    }
  }
  const headerSpan = document.querySelector('.compare-active > div:first-child span');
