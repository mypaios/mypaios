...
async function _checkUnprobed() {
  ...
  for (const m of unprobed) {
    try {
      ...
      fail++;
      if (uiModule) uiModule.showToast(`A model probe failed: ${e.message || 'network error'}`, 5000);
    }
  }
  // Always summarize so the Probe button stopping is never silent.
  if (uiModule) {
    if (fail === 0) uiModule.showToast(`${ok} model${ok > 1 ? 's' : ''} verified`);
    else uiModule.showToast(`Probe done — ${ok} ok · ${fail} failed`, 5000);
  }
  } finally {
