...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    // honor the persisted preference instead of forcing blind
    try { state._blindMode = localStorage.getItem('compare-blind') === '1'; } catch (e) { state._blindMode = false; }
    const blindBtn = document.createElement('button');
    blindBtn.type = 'button';
    blindBtn.className = 'compare-blind-toggle' + (state._blindMode ? ' active' : '');
    ...
    const _selKey = 'paios-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('paios-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('paios-compare-synth-' + state._compareMode);
    ...
    function cleanup(result) {
      ...
      if (result) {
        ...
        state._timeout = Math.min(300, Math.max(5, parseInt(timeoutInput.value) || 30));
    ...
    startBtn.addEventListener('click', async () => {
      ...
      function _updateRow(idx, result) {
        ...
        if (result.status === 'ok') {
          ...
          if (!row._probeTimeout) row._probeTimeout = 120000;
          if (result.error === 'Timeout') row._probeTimeout = Math.min(row._probeTimeout * 2, 240000);
      ...
      try {
        // Probe all in parallel (with 120s timeout per model)
        const results = await Promise.all(modelsToProbe.map(m =>
          Promise.race([
            _probeOne(m),
            new Promise(r => setTimeout(() => r({ status: 'fail', error: 'Timeout' }), 120000))
          ])
        ));
