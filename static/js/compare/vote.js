...
function handleVote(winnerIdx) {
  ...
  if (winnerIdx === -2) {
    for (let i = 0; i < state._selectedModels.length; i++) {
      ...
      const voteLbl = paneEl && paneEl.querySelector('.pane-vote-label');
      if (voteLbl) voteLbl.textContent = 'Vote ' + displayNames[i];
    }
    // Names are out — drop blind mode so vote bar / new panes stay consistent.
    state._blindMode = false;
    try { localStorage.setItem('compare-blind', '0'); } catch (e) {}
