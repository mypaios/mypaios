...
export function _buildServeCmd(f, modelName, backend) {
  ...
  if (backend === 'vllm') {
    ...
    if (f.speculative) {
      ...
      const _draftModel = (f.spec_draft_model || '').trim();
      if (_draftModel) {
        cmd += ` --speculative-model ${_draftModel} --num-speculative-tokens ${_specToks}`;
      } else {
    ...
    if (f.llama_speculative) {
      const _method = f.llama_spec_method || 'mtp';
      const specTokens = parseInt(f.llama_spec_tokens, 10);
      const specN = Number.isFinite(specTokens) && specTokens > 0 ? specTokens : 3;
      if (_method === 'draft') {
        const _draftModel = (f.llama_spec_draft_model || '').trim();
        if (_draftModel) {
          _lcExtra += ` --model-draft "${_draftModel}" --spec-draft-n-max ${specN}`;
        }
      } else {
  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false) {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside MyPaiOS.');
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('MyPaiOS app', 'Run inside the MyPaiOS app itself.', _appDeps),
...
function _renderRecipes() {
  ...
  html += '<div class="hwfit-list-footer" style="margin-top:8px;padding-top:6px;border-top:1px solid color-mix(in srgb, var(--border) 50%, transparent);font-size:9.5px;opacity:0.65;text-align:right;">'
       + 'Don\'t see a model? '
       + '<a href="/help#cookbook" target="_blank" rel="noopener" style="color:var(--accent,var(--red));text-decoration:none;display:inline-flex;align-items:center;gap:4px;vertical-align:middle;">'
       + 'Request it →'
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend MyPaiOS capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install MyPaiOS keys, choose model directories, and set the default server. Local is this machine.</p>';
