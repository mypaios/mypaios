...
function _rerenderCachedModels() {
  ...
  list.querySelectorAll('.memory-item[data-repo]').forEach(item => {
    item.addEventListener('click', (e) => {
      ...
      const _llamaSpecMethod = sv('llama_spec_method', 'mtp');
      const _llamaSpecDraftModel = sv('llama_spec_draft_model', '');
      const _llamaSpecTokens = sv('llama_spec_tokens', '3');

      const _llamaRecs = [
        { val: 'models/qwen2.5-1.5b-instruct-q8_0.gguf', name: 'Qwen 2.5 1.5B Instruct Q8' },
        { val: 'models/qwen2.5-coder-1.5b-instruct-q8_0.gguf', name: 'Qwen 2.5 Coder 1.5B Q8' },
        { val: 'models/llama-3.2-1b-instruct-q8_0.gguf', name: 'Llama 3.2 1B Instruct Q8' }
      ];
      const _llamaSelectedSelect = _llamaSpecDraftModel ? (_llamaRecs.some(r => r.val === _llamaSpecDraftModel) ? _llamaSpecDraftModel : '__custom__') : '';

      let _llamaDraftSelectHtml = `<select class="hwfit-sf hwfit-spec-draft-select" data-field="llama_spec_draft_select" style="width:140px;display:none;margin-left:4px;">`;
      _llamaDraftSelectHtml += `<option value="">Choose GGUF...</option>`;
      _llamaDraftSelectHtml += `<optgroup label="Recommended Fast GGUFs">`;
      _llamaRecs.forEach(r => {
        _llamaDraftSelectHtml += `<option value="${esc(r.val)}"${r.val === _llamaSelectedSelect ? ' selected' : ''}>${esc(r.name)}</option>`;
      });
      _llamaDraftSelectHtml += `</optgroup>`;
      _llamaDraftSelectHtml += `<option value="__custom__"${_llamaSelectedSelect === '__custom__' ? ' selected' : ''}>Custom...</option>`;
      _llamaDraftSelectHtml += `</select>`;

      panelHtml += `<label class="hwfit-sf-cb hwfit-spec-group"><input type="checkbox" class="hwfit-sf" data-field="llama_speculative" /> Speculative <select class="hwfit-sf hwfit-spec-method" data-field="llama_spec_method" style="width:80px;"><option value="mtp"${_llamaSpecMethod==='mtp'?' selected':''}>mtp</option><option value="draft"${_llamaSpecMethod==='draft'?' selected':''}>draft</option></select>${_llamaDraftSelectHtml}<input type="text" class="hwfit-sf hwfit-spec-draft-input" data-field="llama_spec_draft_model" value="${esc(_llamaSpecDraftModel)}" placeholder="Type GGUF path..." style="width:140px;display:none;margin-left:4px;" /><span class="hwfit-numstep" style="margin-left:4px;"><button type="button" class="hwfit-numstep-btn" data-step="-1" tabindex="-1" aria-label="Decrease">‹</button><input type="number" class="hwfit-sf hwfit-spec-tokens" data-field="llama_spec_tokens" value="${esc(_llamaSpecTokens)}" min="1" max="10" title="--spec-draft-n-max" /><button type="button" class="hwfit-numstep-btn" data-step="1" tabindex="-1" aria-label="Increase">›</button></span></label>`;
      panelHtml += `</div>`;
      ...
      if (_opts2.flags.some(f => f.includes('--reasoning-parser'))) { const rp = _opts2.flags.find(f => f.includes('--reasoning-parser')).split(' ')[1]; panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="reasoning_parser" data-parser="${rp}" /> Reasoning Parser <span class="hwfit-parser-tag">${rp}</span></label>`; }
      {
        ...
        const _specDraftModel = sv('spec_draft_model', '');

        const _recs = [
          { val: 'Qwen/Qwen2.5-1.5B-Instruct', name: 'Qwen 2.5 1.5B Instruct' },
          { val: 'Qwen/Qwen2.5-Coder-1.5B-Instruct', name: 'Qwen 2.5 Coder 1.5B Instruct' },
          { val: 'meta-llama/Llama-3.2-1B-Instruct', name: 'Llama 3.2 1B Instruct' },
          { val: 'meta-llama/Llama-3.2-3B-Instruct', name: 'Llama 3.2 3B Instruct' },
          { val: 'google/gemma-2-2b-it', name: 'Gemma 2 2B IT' }
        ];
        const _readyModels = Array.isArray(_cachedAllModels) ? _cachedAllModels.filter(m => m.status === 'ready' && m.repo_id) : [];
        const _isRecOrReady = _recs.some(r => r.val === _specDraftModel) || _readyModels.some(m => m.repo_id === _specDraftModel);
        const _selectedSelect = _specDraftModel ? (_isRecOrReady ? _specDraftModel : '__custom__') : '';

        let _draftSelectHtml = `<select class="hwfit-sf hwfit-spec-draft-select" data-field="spec_draft_select" style="width:140px;display:none;margin-left:4px;">`;
        _draftSelectHtml += `<option value="">Choose draft...</option>`;
        _draftSelectHtml += `<optgroup label="Recommended Fast LLMs">`;
        _recs.forEach(r => {
          _draftSelectHtml += `<option value="${esc(r.val)}"${r.val === _selectedSelect ? ' selected' : ''}>${esc(r.name)}</option>`;
        });
        _draftSelectHtml += `</optgroup>`;
        if (_readyModels.length > 0) {
          _draftSelectHtml += `<optgroup label="Downloaded Models">`;
          _readyModels.forEach(m => {
            _draftSelectHtml += `<option value="${esc(m.repo_id)}"${m.repo_id === _selectedSelect ? ' selected' : ''}>${esc(m.repo_id.split('/').pop())}</option>`;
          });
          _draftSelectHtml += `</optgroup>`;
        }
        _draftSelectHtml += `<option value="__custom__"${_selectedSelect === '__custom__' ? ' selected' : ''}>Custom...</option>`;
        _draftSelectHtml += `</select>`;

        const _specMethods = ['mtp', 'qwen3_next_mtp', 'eagle', 'medusa', 'ngram', 'draft'];
      ...
      function updateBackendVisibility() {
        ...
        // Show/hide vLLM speculative draft input/select
        const isVllmSpec = panel.querySelector('[data-field="speculative"]')?.checked;
        const vllmMethod = panel.querySelector('[data-field="spec_method"]')?.value || 'mtp';
        const vllmDraftSelect = panel.querySelector('[data-field="spec_draft_select"]');
        const vllmDraftInput = panel.querySelector('[data-field="spec_draft_model"]');
        if (vllmDraftSelect) {
          vllmDraftSelect.style.display = (b === 'vllm' && isVllmSpec && vllmMethod === 'draft') ? 'inline-block' : 'none';
        }
        if (vllmDraftInput) {
          vllmDraftInput.style.display = (b === 'vllm' && isVllmSpec && vllmMethod === 'draft' && vllmDraftSelect?.value === '__custom__') ? 'inline-block' : 'none';
        }

        // Show/hide llama.cpp speculative draft input/select
        const isLlamaSpec = panel.querySelector('[data-field="llama_speculative"]')?.checked;
        const llamaMethod = panel.querySelector('[data-field="llama_spec_method"]')?.value || 'mtp';
        const llamaDraftSelect = panel.querySelector('[data-field="llama_spec_draft_select"]');
        const llamaDraftInput = panel.querySelector('[data-field="llama_spec_draft_model"]');
        if (llamaDraftSelect) {
          llamaDraftSelect.style.display = (b === 'llamacpp' && isLlamaSpec && llamaMethod === 'draft') ? 'inline-block' : 'none';
        }
        if (llamaDraftInput) {
          llamaDraftInput.style.display = (b === 'llamacpp' && isLlamaSpec && llamaMethod === 'draft' && llamaDraftSelect?.value === '__custom__') ? 'inline-block' : 'none';
        }
      }
      ...
      function _loadSlotIntoPanel(slotIdx) {
        ...
        if (p.fields) {
          ...
          const fields = {
            ...
            llama_spec_method: cmd.includes('--model-draft') || cmd.includes('-md') ? 'draft' : 'mtp',
            llama_spec_draft_model: _ex(/(?:--model-draft|-md)\s+["']?([^"'\s]+)["']?/) || '',
            spec_draft_model: _ex(/--speculative-model\s+(\S+)/) || '',
            venv: p.envPath || '',
          };
          const checks = {
            ...
            llama_speculative: /--spec-type\s+\S*draft-mtp/.test(cmd) || cmd.includes('--model-draft') || cmd.includes('-md'),
            speculative: cmd.includes('--speculative-config') || cmd.includes('--speculative-model'),
          };
          ...
          const _specModelMatch = cmd.match(/--speculative-model\s+(\S+)(?:\s+--num-speculative-tokens\s+(\d+))?/);
          if (_specModelMatch) {
            fields.spec_draft_model = _specModelMatch[1];
            fields.spec_method = 'draft';
            if (_specModelMatch[2]) fields.spec_tokens = _specModelMatch[2];
          }

          const _recs = ['Qwen/Qwen2.5-1.5B-Instruct', 'Qwen/Qwen2.5-Coder-1.5B-Instruct', 'meta-llama/Llama-3.2-1B-Instruct', 'meta-llama/Llama-3.2-3B-Instruct', 'google/gemma-2-2b-it'];
          const _readyModels = Array.isArray(_cachedAllModels) ? _cachedAllModels.filter(m => m.status === 'ready' && m.repo_id).map(m => m.repo_id) : [];
          if (fields.spec_draft_model) {
            const isKnown = _recs.includes(fields.spec_draft_model) || _readyModels.includes(fields.spec_draft_model);
            fields.spec_draft_select = isKnown ? fields.spec_draft_model : '__custom__';
          } else {
            fields.spec_draft_select = '';
          }

          const _llamaRecs = ['models/qwen2.5-1.5b-instruct-q8_0.gguf', 'models/qwen2.5-coder-1.5b-instruct-q8_0.gguf', 'models/llama-3.2-1b-instruct-q8_0.gguf'];
          if (fields.llama_spec_draft_model) {
            const isKnown = _llamaRecs.includes(fields.llama_spec_draft_model);
            fields.llama_spec_draft_select = isKnown ? fields.llama_spec_draft_model : '__custom__';
          } else {
            fields.llama_spec_draft_select = '';
          }
          panel.querySelectorAll('.hwfit-sf').forEach(el => {
            const f = el.dataset.field;
      ...
      // reflects the stored presets. Standard MyPaiOS .dropdown look, positioned
      ...
      panel.querySelectorAll('.hwfit-sf').forEach(el => {
        ...
        el.addEventListener('change', (e) => {
          ...
          if (f === 'spec_draft_select') {
            const val = e.target.value;
            const input = panel.querySelector('[data-field="spec_draft_model"]');
            if (input) {
              if (val === '__custom__') {
                input.value = '';
              } else {
          ...
          if (f === 'llama_spec_draft_select') {
            const val = e.target.value;
            const input = panel.querySelector('[data-field="llama_spec_draft_model"]');
            if (input) {
              if (val === '__custom__') {
                input.value = '';
              } else {
          ...
          if (f === 'speculative' || f === 'spec_method' ||
              f === 'llama_speculative' || f === 'llama_spec_method') {
            updateBackendVisibility();
          }
          if (f === 'venv') {
            updateRuntimeReadinessNote();
