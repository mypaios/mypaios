...
const _acct = () => window.__paiosActiveEmailAccount
  ? `&account_id=${encodeURIComponent(window.__paiosActiveEmailAccount)}`
  : '';
...
function _bindEvents() {
  ...
  const header = section?.querySelector('.section-header-flex') || section;
  if (header) {
    header.style.cursor = 'pointer';
    header.addEventListener('click', (e) => {
      if (e.target.closest('#email-compose-btn')) return;
      if (e.target.closest('#email-guard-btn')) return; // Mail Guard has its own panel
      openEmailLibrary();
      markInboxAsSeen();
    });
  }
...
async function _refreshUnreadCount() {
  ...
  try {
    ...
    const _seenKey = 'paios-email-last-seen-uid:' + (window.__paiosActiveEmailAccount || 'default');
    const lastSeen = parseInt(localStorage.getItem(_seenKey) || localStorage.getItem('paios-email-last-seen-uid') || '0', 10);
  ...
  try {
    ...
    fetch(`${API_BASE}/api/email/list?folder=INBOX&limit=1${_acct()}`)
      ...
      .then(data => {
        ...
        if (emails.length > 0) {
          const maxUid = Math.max(...emails.map(e => parseInt(e.uid, 10) || 0));
          localStorage.setItem('paios-email-last-seen-uid:' + (window.__paiosActiveEmailAccount || 'default'), String(maxUid));
        }
        const dot = document.getElementById('email-unread-dot');
        if (dot) dot.style.display = 'none';
      })
...
function _createEmailItem(em) {
  ...
  if (unflagBtn) {
    unflagBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      try {
        const r = await fetch(`${API_BASE}/api/email/${em.uid}/unflag-spam`, {
          method: 'POST', credentials: 'same-origin',
        });
        if (!r.ok) { import('./ui.js').then(m => m.showError && m.showError('Could not unflag')).catch(() => {}); return; }
        em.is_spam_verdict = false;
        item.classList.remove('email-item-spam');
        const tag = item.querySelector('.email-tag-spam');
        if (tag) tag.remove();
      } catch (_) { import('./ui.js').then(m => m.showError && m.showError('Could not unflag')).catch(() => {}); }
    });
  }
...
async function _openEmail(em, itemEl, preloadedData = null, mode = 'reply') {
  ...
  try {
    ...
    if (data.error) {
      ...
      import('./ui.js').then(m => m.showError && m.showError('Could not open email: ' + data.error)).catch(() => {});
      return;
    }
    if (wantsAiReply) {
...
async function _archiveEmail(em) {
  try {
    const r = await fetch(`${API_BASE}/api/email/archive/${em.uid}?folder=${encodeURIComponent(_currentFolder)}${_acct()}`, { method: 'POST' });
    if (!r.ok) { import('./ui.js').then(m => m.showError && m.showError('Archive failed')).catch(() => {}); return; }
    _emails = _emails.filter(e => e.uid !== em.uid);
    _renderList();
  } catch (e) {
    console.error('Failed to archive:', e);
    import('./ui.js').then(m => m.showError && m.showError('Archive failed — is the server up?')).catch(() => {});
  }
}
...
async function _deleteEmail(em) {
  ...
  if (!ok) return;
  try {
    const r = await fetch(`${API_BASE}/api/email/delete/${em.uid}?folder=${encodeURIComponent(_currentFolder)}${_acct()}`, { method: 'DELETE' });
    if (!r.ok) { import('./ui.js').then(m => m.showError && m.showError('Delete failed')).catch(() => {}); return; }
    _emails = _emails.filter(e => e.uid !== em.uid);
    _renderList();
  } catch (e) {
    console.error('Failed to delete:', e);
    import('./ui.js').then(m => m.showError && m.showError('Delete failed — is the server up?')).catch(() => {});
  }
}
...
async function _composeNew() {
  ...
  if (_composing) return;   // guard: compose makes several calls — ignore double-clicks
  _composing = true;
  try {
    await _composeNewInner();
  } finally { setTimeout(() => { _composing = false; }, 1200); }
}

async function _composeNewInner() {
  if (!_docModule) return;
