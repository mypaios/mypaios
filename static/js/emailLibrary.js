...
function _publishActiveAccount() {
  try { window.__paiosActiveEmailAccount = state._libAccountId || null; } catch (_) {}
...
export function openEmailLibrary(opts = {}) {
    ...
    <div class="modal-content doclib-modal-content" style="width:min(720px, 92vw);max-height:85vh;background:var(--bg);">
      <div class="modal-header">
        ...
        <div class="email-lib-header-actions" style="display:flex;align-items:center;gap:8px;">
          <button class="close-btn" id="email-lib-close" title="Close (Esc)">\u2716</button>
        </div>
      </div>
        ...
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Permanently delete MyPaiOS reminder emails">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Show MyPaiOS reminder emails">
  ...
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Permanently delete all MyPaiOS reminder emails?', {
      confirmText: 'Delete',
      cancelText: 'Cancel',
      danger: true,
    });
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/email/paios/reminders?permanent=1${_acct()}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
...
function _renderAccountsStrip() {
  ...
  let html = `<button class="memory-toolbar-btn gallery-chip${allActive}" data-acc-id="" title="Your default mailbox (not an all-accounts view)">Default</button>`;
...
const _BUBBLES_DISABLED_KEY = 'paios.email.bubblesDisabled';
...
const _SUMMARY_COLLAPSED_KEY = 'paios.email.summaryCollapsed';
...
function _showBulkActionsMenu(anchor) {
  ...
  const _archIco = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="5" rx="1"/><path d="M4 8v11a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8"/><line x1="10" y1="12" x2="14" y2="12"/></svg>';
  const items = [
    ...
    { label: 'Archive', icon: _archIco, action: () => _bulkAction('archive') },
  ];
...
function _showLibRemindSubmenu(em, parentDropdown) {
  ...
  customItem.addEventListener('click', (e) => {
    ...
    // Same fix as emailInbox._showRemindSubmenu: blur fires when the native
    // datetime popup steals focus, killing the input before a date could be
    // picked. Dismiss on click-outside / Escape instead.
    const _cleanup = () => {
      tmp.remove();
...
async function _createEmailReplyReminder(em, dueDate) {
  ...
  const fullFrom = em.from || em.sender || em.from_name || em.from_address || '';
