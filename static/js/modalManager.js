...
const _rememberedDockKey = (id) => `paios-modal-remembered-dock-${id}`;
...
function _emitModalOpened(id, modal) {
  try {
    window.dispatchEvent(new CustomEvent('paios:modal-opened', {
      detail: { id, modal },
    }));
  } catch (_) {}
}
...
const _DOCK_STORAGE_KEY = 'paios.mobileDockState.v1';
