...
let _searchQuery = '';
let _viewMode = (typeof localStorage !== 'undefined' && localStorage.getItem('paios-notes-view')) || 'list'; // 'list' or 'grid'
...
const REMINDER_FIRED_KEY = 'paios-notes-reminder-fired';
...
const REMINDER_GLOWED_KEY = 'paios-notes-reminder-glowed';
...
const REMINDER_PENDING_HIGHLIGHT_KEY = 'paios-notes-reminder-pending-highlight';
const REMINDER_ACTIVE_HIGHLIGHT_KEY = 'paios-notes-reminder-active-highlight';
...
const REMINDER_DISMISSED_AT_KEY = 'paios-notes-reminder-dismissed-at';
const NOTES_FIRST_OPEN_HINT_KEY = 'paios-notes-first-open-hint-v1';
...
export function openPanel() {
  ...
  if (viewBtn) {
    ...
    viewBtn.addEventListener('click', () => {
      ...
      try { localStorage.setItem('paios-notes-view', _viewMode); } catch {}
      pane.classList.toggle('notes-view-grid', _viewMode === 'grid');
      _setViewLabel();
      requestAnimationFrame(() => _applyMasonry(document.querySelector('#notes-pane .notes-pane-body')));
    });
...
function _renderNotes() {
  ...
  for (const note of sorted) {
    ...
    const goalPill = note.note_type === 'goal'
      ...
      : '';
    html += `<div class="note-card${note.pinned ? ' note-card-pinned' : ''}${cc}${sel}${goalClass}${reminderGlowClass}${_selectMode ? ' note-card-selectmode' : ''}" draggable="false" data-note-id="${note.id}"${cardStyle}>
    ...
    body.querySelectorAll('.note-card').forEach(card => {
      ...
      if (header) {
        header.addEventListener('mousedown', (e) => {
          if (e.target.closest('button, input, select, a, .note-card-pin, .note-card-done')) return;
          card.draggable = true;
        });
...
const _DRAFT_PREFIX = 'paios-note-draft-';
...
function _buildChecklistHtml(items) {
  ...
  for (const item of items) {
    const indent = Math.min(item.indent || 0, 3);
    html += `<div class="note-cl-row${item.done ? ' done' : ''}" draggable="false" data-item-id="${item.id || _uid()}" data-indent="${indent}" style="padding-left:${indent * 16}px">
...
function _wireRow(row, container) {
  ...
  // Drag handlers
  const grip = row.querySelector('.note-cl-grip');
  if (grip) {
