  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('paios-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('paios-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('paios-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('paios-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('paios-char-sessions', JSON.stringify(charSessions));
