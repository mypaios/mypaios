...
const _SETTINGS_KEY = 'paios-research-settings';
const _COLLAPSE_KEY = 'paios-research-settings-collapsed';
  ...
  return `
    ...
    <div class="modal-body research-pane-body" data-no-swipe-dismiss>
      <div class="research-new-job">
        ...
        <textarea id="research-query" class="research-query" placeholder="e.g. Trace Pi's ten-year journey home from Troy — every island, monster, and detour, and why each one cost him" rows="4"></textarea>
        ...
        <div id="research-settings-body" class="research-settings-row"${settingsHidden}>
          <label class="research-setting" title="Maximum research rounds (a ceiling). Auto lets Pi decide how deep to go.">
            <span class="research-setting-label">Max rounds</span>
...
function _wireEvents(pane) {
  ...
  pane.querySelector('#research-settings-toggle').addEventListener('click', () => {
    ...
    try { localStorage.setItem('paios-research-settings-collapsed', _settingsCollapsed ? '1' : '0'); } catch {}
  });
...
// Open the visual report in an IN-APP iframe modal. The report route is
// owner-scoped (needs the session), and a new tab / Electron-external window
// doesn't carry the cookie (the "Not authenticated" error). A same-origin
// iframe in this document does — so we render it inline. The iframe src is
// RELATIVE so it always matches the app's origin (avoids the localhost vs
// 127.0.0.1 cookie split too).
let _reportModalOpen = false;
function _openReportModal(id) {
  if (_reportModalOpen) return;
  _reportModalOpen = true;
  const url = `/api/research/report/${id}`;
  const modal = document.createElement('div');
  modal.className = 'modal';
  modal.id = 'research-report-modal';
  modal.innerHTML = `
    <div class="modal-content" style="max-width:1100px;width:97vw;height:92vh;display:flex;flex-direction:column;padding:0;overflow:hidden;">
      <div style="display:flex;align-items:center;gap:10px;padding:9px 14px;border-bottom:1px solid var(--border);flex-shrink:0;">
        <span style="font-weight:600;flex:1;font-size:14px;">🔍 Visual Report</span>
        <button class="admin-btn-sm" id="research-report-newtab" title="Open in a new tab">↗</button>
        <button class="close-btn" id="research-report-close">✖</button>
      </div>
      <iframe src="${url}" style="flex:1;border:none;width:100%;background:var(--bg);" title="Research report"></iframe>
    </div>`;
  ...
  modal.querySelector('#research-report-close').addEventListener('click', close);
  modal.addEventListener('mousedown', (e) => { if (e.target === modal) close(); });
  modal.querySelector('#research-report-newtab').addEventListener('click', () => window.open(url, '_blank'));
