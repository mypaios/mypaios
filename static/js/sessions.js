...
let _initialSessionSelected = false;
...
let _sortMode = Storage.get('paios-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'MyPaiOS Chat';
  Storage.set('lastSessionId', 'none');
...
const FOLDER_STATE_KEY = 'paios-folder-state';
const FOLDER_ORDER_KEY = 'paios-folder-order';
...
function _postRenderSessionList(list) {
  if (currentSessionId) {
    ...
    if (activeEl) {
      ...
      if (_sessionListFocused) activeEl.focus({ preventScroll: true });
    }
  }
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    if (closeBtn) closeBtn.style.display = id ? 'inline-flex' : 'none';
    if (window.expandSection) {
      if (_initialSessionSelected) {
        window.expandSection('sessions-section');
      } else {
        _initialSessionSelected = true;
      }
    }
    ...
    // URL hash — the user complained that coming back to MyPaiOS kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'MyPaiOS Chat';
    }
    ...
    // Pinned (not one-shot) — async content finishing after load used to
    // leave old chats stranded mid-history.
    uiModule.scrollHistoryPinned();
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('paios-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('paios-session-sort', mode);
  else Storage.remove('paios-session-sort');
  renderSessionList();
}
