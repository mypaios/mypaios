...
// Download a skill as a portable .zip (SKILL.md + scripts/references/assets).
// Uses a same-origin anchor so the session cookie authorizes the export route.
function _exportSkill(name) {
  const a = document.createElement('a');
  a.href = `${API}/api/skills/${encodeURIComponent(name)}/export`;
  a.download = `${name}.zip`;
...
// Import a skill package (.zip). The backend validates SKILL.md, refuses
// zip-slip, and QUARANTINES it (source=imported, trusted_exec=false) so its
// bundled scripts can't run until the user reviews and trusts it.
async function _importSkillZip(file) {
  if (!file) return;
  const fd = new FormData();
  fd.append('file', file);
  try {
    const res = await fetch(`${API}/api/skills/import`, { method: 'POST', body: fd });
    const d = await res.json().catch(() => ({}));
    if (!res.ok || !d.ok) throw new Error(d.detail || d.error || `HTTP ${res.status}`);
    uiModule.showToast(`Imported “${d.name}” (${d.files} file${d.files === 1 ? '' : 's'}) — quarantined; review before running its scripts.`);
...
function _openSkillMenu(btn, card, sk, name, isPublished) {
  ...
  mk('<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/>',
     'Export .zip', {}, () => _exportSkill(name));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('skills-import-btn')?.addEventListener('click', () => document.getElementById('skills-import-file')?.click());
  document.getElementById('skills-import-file')?.addEventListener('change', (e) => {
