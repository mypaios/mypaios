...
export const THEMES = {
  dark:       { bg:'#2E333D', fg:'#EDF3F7', panel:'#21252C', border:'#3D547B', red:'#FF5758' },
  light:      { bg:'#F5FAFD', fg:'#2E333D', panel:'#FFFFFF', border:'#D9D9D9', red:'#FF5758' },
  ...
  'claude-light': { bg:'#f5f4ee', fg:'#28261f', panel:'#ffffff', border:'#e3e0d6', red:'#c96442',
                advanced: { userBubbleBg:'#ffffff', aiBubbleBg:'#f5f4ee', bubbleBorder:'#e3e0d6',
                            sidebarBg:'#ece9e0', brandColor:'#c96442', hamburgerColor:'#28261f',
                            inputBg:'#ffffff', inputBorder:'#dcd8cc', sendBtnBg:'#c96442', sendBtnHover:'#b5512f',
                            codeBg:'#f0eee6', codeFg:'#28261f', toggleActive:'#c96442' } },
  cute:       { bg:'#fff0f5', fg:'#d4608a', panel:'#fff8fa', border:'#f0c0d0', red:'#ff6b9d' },
  antigravity:{ bg:'#0A0B14', fg:'#D7DBF0', panel:'#11131F', border:'#2E3358', red:'#8B7BFF', green:'#42E2B8' },
};

const DEFAULT_THEME = 'dark';
const LS_KEY = 'paios-theme';
const CUSTOM_THEMES_KEY = 'paios-custom-themes';
...
// Per-theme typography defaults (font key from FONT_MAP + density). Lets a
// built-in theme ship a look-and-feel, not just colors — e.g. claude-light
// uses a clean sans at the spacious size (16px + roomy padding) to match
// Claude's reading feel. Absent themes fall back to DEFAULT_FONT/DENSITY.
const THEME_DEFAULT_FONT = {
  'claude-light': 'sans',
  claude:         'sans',
};
const THEME_DEFAULT_DENSITY = {
  'claude-light': 'spacious',
  claude:         'spacious',
};
...
const ADV_KEYS = [
  ...
  { key: 'brandColor',         css: '--brand-color',       label: 'MyPaiOS Logo',    group: 'Sidebar' },
...
export function applyColors(colors) {
  ...
  // Secondary surfaces + muted text, derived from THIS theme's own colors so
  // they adapt to every theme (light, dark, custom). Many JS-built panels
  // (Code bubbles, Activity, Workspace, Storage, Agents, Mail Guard, Local AI)
  // reference these; without them they fell back to the dark stylesheet value
  // and rendered dark-on-dark in light themes. Mixing toward --fg/--bg keeps
  // the contrast direction correct regardless of whether the theme is light or
  // dark. --mono is a font (theme-independent) and stays in style.css.
  s.setProperty('--panel2', 'color-mix(in srgb, var(--panel) 89%, var(--fg))');
  s.setProperty('--bg2', 'color-mix(in srgb, var(--panel) 95%, var(--fg))');
  s.setProperty('--muted', 'color-mix(in srgb, var(--fg) 70%, var(--bg))');
...
export function applyBgPattern(pattern) {
  let p = pattern || 'none';
  // Reduce-motion globally suppresses animated/canvas backgrounds.
  if (document.documentElement.classList.contains('reduce-motion')) p = 'none';
...
// ── Global display preferences (independent of theme presets) ───────────────
// These are accessibility/comfort prefs that apply across every theme, so they
// live in their own localStorage keys, not in the per-theme object.
const SCALE_KEY = 'paios-display-scale';
const MOTION_KEY = 'paios-reduce-motion';

export function getDisplayScale() {
  try { return localStorage.getItem(SCALE_KEY) || '1'; } catch (e) { return '1'; }
}
export function applyDisplayScale(scale) {
  let z = parseFloat(scale);
  if (!z || z < 0.6 || z > 2.5) z = 1;
  // CSS `zoom` scales the WHOLE interface — incl. the ~1100 hardcoded-px sizes
  // a font-size change can't touch — so it's the only reliable "make everything
  // bigger" lever. Supported in Chromium (Electron) + WebKit.
...
export function applyReduceMotion(on) {
  ...
  // Re-apply the saved background so canvas effects start/stop accordingly
  // (applyBgPattern forces 'none' while reduce-motion is on).
  try { const sv = getSaved(); applyBgPattern((sv && sv.bgPattern) || 'none'); } catch (e) {}
}
...
export function initThemeUI() {
  ...
  grid.innerHTML = Object.entries(THEMES).map(([name, c]) => `
    <div class="theme-swatch${name === activeName ? ' active' : ''}" data-theme="${name}">
      ...
      ${name === 'dark' ? 'original' : (name === 'gpt' ? 'GPT' : (name === 'claude-light' ? 'claude light' : name))}
    </div>
  `).join('');
  ...
  allGrids.forEach(g => {
    g.querySelectorAll('.theme-swatch').forEach(sw => {
      sw.addEventListener('click', (e) => {
        ...
        const d = ct && ct.density ? ct.density : (THEME_DEFAULT_DENSITY[name] || DEFAULT_DENSITY);
  ...
  // Global display prefs (own LS keys, independent of theme presets). Set the
  // reduce-motion class BEFORE applyBgPattern so canvas effects honor it.
  const _initScale = getDisplayScale();
  const _initMotion = getReduceMotion();
  document.documentElement.classList.toggle('reduce-motion', _initMotion);
  applyDisplayScale(_initScale);
  ...
  // Display size (whole-UI zoom) + Reduce motion — global prefs, persist on their
  // own (no theme save needed) so they hold across theme switches.
  const scaleSelect = document.getElementById('theme-scale-select');
  if (scaleSelect) {
    const nsc = scaleSelect.cloneNode(true); scaleSelect.parentNode.replaceChild(nsc, scaleSelect);
    nsc.value = _initScale;
    nsc.addEventListener('change', () => setDisplayScale(nsc.value));
  }
  const motionToggle = document.getElementById('theme-reducemotion-toggle');
  if (motionToggle) {
    const nm = motionToggle.cloneNode(true); motionToggle.parentNode.replaceChild(nm, motionToggle);
    nm.checked = _initMotion;
    nm.addEventListener('change', () => setReduceMotion(nm.checked));
  }
  if (patternSelect) {
  ...
  if (exportBtnEl) {
    ...
    newExp.addEventListener('click', () => {
      ...
      a.href = url;
      a.download = 'paios_' + (obj.name || 'theme') + '.json';
      a.click();
...
const themeModule = { initThemeUI, togglePopup, closePopup, makeDraggable,
                       ...
                       applyFrostedGlass,
                       applyDisplayScale, setDisplayScale, getDisplayScale,
                       applyReduceMotion, setReduceMotion, getReduceMotion,
