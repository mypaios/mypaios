...
function _initHoverCardSpaceToggle() {
  if (document._paiosHoverCardSpaceToggle) return;
  document._paiosHoverCardSpaceToggle = true;
...
function _smoothScrollStep() {
  ...
  const nextScroll = current + diff * factor;
  box.scrollTop = nextScroll;

  // If scroll position didn't change (e.g. hit subpixel rounding limit), snap and exit
  if (Math.abs(box.scrollTop - current) < 0.5) {
    box.scrollTop = target;
    _scrollRafId = null;
    return;
  }
...
/**
 * Animated scroll to the very bottom, from any distance (the streaming
 * smooth-scroller above deliberately refuses jumps > 300px). rAF lerp —
 * native scrollTo({behavior:'smooth'}) silently no-ops on this container
 * in some Chromium environments. Cancels if the user scrolls up mid-flight.
 */
let _jumpRafId = null;
let _jumpFallback = null;
export function scrollHistoryJumpToBottom() {
  const box = document.getElementById('chat-history');
  if (!box) return;
  const cancel = () => {
    if (_jumpRafId) { cancelAnimationFrame(_jumpRafId); _jumpRafId = null; }
    if (_jumpFallback) { clearTimeout(_jumpFallback); _jumpFallback = null; }
  };
  cancel();
  box.addEventListener('wheel', cancel, { once: true, passive: true });
  box.addEventListener('touchstart', cancel, { once: true, passive: true });
  const step = () => {
    const target = box.scrollHeight - box.clientHeight;
    const diff = target - box.scrollTop;
    if (diff <= 8) { box.scrollTop = target; cancel(); return; }
    box.scrollTop += diff * 0.2;
    _jumpRafId = requestAnimationFrame(step);
  };
  _jumpRafId = requestAnimationFrame(step);
  // rAF is throttled/suspended in background tabs — if the animation didn't
  // finish in time, snap the rest of the way.
  _jumpFallback = setTimeout(() => {
    if (_jumpRafId) { cancelAnimationFrame(_jumpRafId); _jumpRafId = null; }
...
/**
 * Scroll to bottom and KEEP it there briefly. A single instant scroll on
 * session open lands mid-history: images, video digests, mermaid/KaTeX and
 * code highlight all finish after it and grow the page. Re-snap for a short
 * window, backing off the moment the user scrolls up themselves.
 */
let _pinTimer = null;
export function scrollHistoryPinned(ms = 1500) {
  const box = document.getElementById('chat-history');
  if (!box) return;
  if (_pinTimer) { clearInterval(_pinTimer); _pinTimer = null; }
  box.scrollTop = box.scrollHeight;
  const started = performance.now();
  const cancel = () => { if (_pinTimer) { clearInterval(_pinTimer); _pinTimer = null; } };
  box.addEventListener('wheel', cancel, { once: true, passive: true });
  box.addEventListener('touchmove', cancel, { once: true, passive: true });
  _pinTimer = setInterval(() => {
    box.scrollTop = box.scrollHeight;
    if (performance.now() - started > ms) cancel();
  }, 120);
}
...
// (Click-to-front / z-ordering for tool panels is owned by panelCoordinator.js.)
{
...
if (!window._odyEscExpandGuard) {
  ...
  // Z-ordering for tool panels (single authoritative counter + single-active)
  // is owned by static/js/panelCoordinator.js. Here we only need a visibility
  // test for the Escape arbiter's "topmost modal" lookup below.
