...
def test_vllm_preflight_reports_cli_and_version():
    ...
    assert 'PAIOS_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"' in script
    assert 'echo "[paios] vLLM CLI: $PAIOS_VLLM_BIN"' in script
    assert '"$PAIOS_VLLM_BIN" --version' in script
    assert 'PAIOS_PREFLIGHT_EXIT=127' in script
...
def test_serve_preflight_failure_keeps_tmux_pane_visible():
    ...
    assert 'PAIOS_PREFLIGHT_EXIT=127' in script
    assert 'echo "=== Process exited with code $PAIOS_PREFLIGHT_EXIT ==="' in script
...
def test_serve_runner_preserves_command_exit_code():
    ...
    assert "PAIOS_CMD_EXIT=$?" in script
    assert 'echo "=== Process exited with code $PAIOS_CMD_EXIT ==="' in script
...
def test_pip_serve_runner_emits_download_ok_before_exit_marker():
    ...
    assert 'exit "$PAIOS_CMD_EXIT"' in script
...
def test_llama_cpp_linux_bootstrap_checks_cudart_before_cuda_build():
    ...
    assert '_paios_has_cudart' in script
    ...
    assert script.index('_paios_has_cudart') < script.index('DGGML_CUDA=ON')
