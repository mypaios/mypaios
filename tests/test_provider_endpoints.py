...
def test_headers_openrouter_adds_attribution():
    ...
    assert h["Authorization"] == "Bearer secret"
    # OpenRouter ranks/labels apps via these headers. X-Title is the
    # documented one; the legacy X-OpenRouter-Title stays one release.
    assert h["HTTP-Referer"] == "https://github.com/mypaios/mypaios"
    assert h["X-Title"] == "MyPaiOS"
    assert h["X-OpenRouter-Title"] == "MyPaiOS"
