...
def test_ollama_cookbook_runner_does_not_force_public_bind():
    ...
    assert 'OLLAMA_HOST="0.0.0.0:${PAIOS_OLLAMA_PORT}" ollama serve' not in route
    assert 'OLLAMA_HOST="${PAIOS_OLLAMA_HOST}:${PAIOS_OLLAMA_PORT}" ollama serve' in route
...
def test_inprocess_pollers_gate(monkeypatch):
    """The PAIOS_INPROCESS_POLLERS env var must let operators kill
    the asyncio pollers when cron / systemd is driving the one-shot
    `paios-mail poll-*` CLI subcommands instead. Two pollers racing
    ...
    for on in ("1", "true", "yes", "anything-truthy"):
        monkeypatch.setenv("PAIOS_INPROCESS_POLLERS", on)
...
def _load_search_content_for_test(monkeypatch, name="services.search.content_under_test"):
    ...
    cache.CONTENT_CACHE_DIR = Path("/tmp/paios-test-content-cache")
